...
func snapshotRestoreCommandFunc(cmd *cobra.Command, args []string) {
	...
	sp := snapshot.NewV3(lg)
	if err := sp.Restore(snapshot.RestoreConfig{SnapshotPath: args[0], Name: restoreName, OutputDataDir: dataDir, OutputWALDir: walDir, PeerURLs: strings.Split(restorePeerURLs, ","), InitialCluster: restoreCluster, InitialClusterToken: restoreClusterToken, SkipHashCheck: skipHashCheck}); err != nil {
		ExitWithError(ExitError, err)
	}
}
