...
func (p *printerUnsupported) EndpointHealth([]epHealth) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	p.p(nil)
}
...
func (p *printerUnsupported) DBStatus(snapshot.Status) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	p.p(nil)
}
...
func makeEndpointHealthTable(healthList []epHealth) (hdr []string, rows [][]string) {
	...
	for _, h := range healthList {
		rows = append(rows, []string{h.Ep, fmt.Sprintf("%v", h.Health), h.Took, h.Error})
	}
	return hdr, rows
}
func makeEndpointStatusTable(statusList []epStatus) (hdr []string, rows [][]string) {
	...
	for _, status := range statusList {
		rows = append(rows, []string{status.Ep, fmt.Sprintf("%x", status.Resp.Header.MemberId), status.Resp.Version, humanize.Bytes(uint64(status.Resp.DbSize)), fmt.Sprint(status.Resp.Leader == status.Resp.Header.MemberId), fmt.Sprint(status.Resp.RaftTerm), fmt.Sprint(status.Resp.RaftIndex), fmt.Sprint(status.Resp.RaftAppliedIndex), fmt.Sprint(strings.Join(status.Resp.Errors, ", "))})
	}
	return hdr, rows
}
