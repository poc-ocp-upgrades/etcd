...
func TestTransportSend(t *testing.T) {
	...
	wmsgsIgnored := []raftpb.Message{{Type: raftpb.MsgBeat}, {Type: raftpb.MsgProp, To: 3}}
	wmsgsTo1 := []raftpb.Message{{Type: raftpb.MsgProp, To: 1}, {Type: raftpb.MsgApp, To: 1}}
	wmsgsTo2 := []raftpb.Message{{Type: raftpb.MsgProp, To: 2}, {Type: raftpb.MsgApp, To: 2}}
...
func TestTransportCutMend(t *testing.T) {
	...
	wmsgsTo := []raftpb.Message{{Type: raftpb.MsgProp, To: 1}, {Type: raftpb.MsgApp, To: 1}}
	tr.Send(wmsgsTo)
	if len(peer1.msgs) > 0 {
...
func TestTransportAdd(t *testing.T) {
	...
	tr := &Transport{LeaderStats: ls, streamRt: &roundTripperRecorder{}, peers: make(map[types.ID]Peer), pipelineProber: probing.NewProber(nil), streamProber: probing.NewProber(nil)}
...
func TestTransportUpdate(t *testing.T) {
	...
	defer _logClusterCodePath()
	peer := newFakePeer()
	tr := &Transport{peers: map[types.ID]Peer{types.ID(1): peer}, pipelineProber: probing.NewProber(nil), streamProber: probing.NewProber(nil)}
...
func TestTransportErrorc(t *testing.T) {
	...
	tr := &Transport{Raft: &fakeRaft{}, LeaderStats: stats.NewLeaderStats(""), ErrorC: errorc, streamRt: newRespRoundTripper(http.StatusForbidden, nil), pipelineRt: newRespRoundTripper(http.StatusForbidden, nil), peers: make(map[types.ID]Peer), pipelineProber: probing.NewProber(nil), streamProber: probing.NewProber(nil)}
	tr.AddPeer(1, []string{"http://localhost:2380"})
	defer tr.Stop()
	select {
	case <-errorc:
