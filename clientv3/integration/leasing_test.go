...
func TestLeasingGetWithOpts(t *testing.T) {
	...
	clus.Members[0].Stop(t)
	opts := []clientv3.OpOption{clientv3.WithKeysOnly(), clientv3.WithLimit(1), clientv3.WithMinCreateRev(1), clientv3.WithMinModRev(1), clientv3.WithSort(clientv3.SortByKey, clientv3.SortAscend), clientv3.WithSerializable()}
...
func TestLeasingTxnOwnerGet(t *testing.T) {
	...
	tresp, terr := lkv.Txn(context.TODO()).If(cmps...).Then(thenOps...).Else(elseOps...).Commit()
	if terr != nil {
		t.Fatal(terr)
	}
...
func TestLeasingTxnOwnerIf(t *testing.T) {
	...
	tests := []struct {
		...
		wSucceeded	bool
		wResponses	int
	}{{cmps: []clientv3.Cmp{clientv3.Compare(clientv3.Value("k"), "=", "abc")}, wSucceeded: true, wResponses: 1}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.CreateRevision("k"), "=", 2)}, wSucceeded: true, wResponses: 1}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.ModRevision("k"), "=", 2)}, wSucceeded: true, wResponses: 1}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.Version("k"), "=", 1)}, wSucceeded: true, wResponses: 1}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.Value("k"), ">", "abc")}}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.CreateRevision("k"), ">", 2)}}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.ModRevision("k"), "=", 2)}, wSucceeded: true, wResponses: 1}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.Version("k"), ">", 1)}}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.Value("k"), "<", "abc")}}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.CreateRevision("k"), "<", 2)}}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.ModRevision("k"), "<", 2)}}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.Version("k"), "<", 1)}}, {cmps: []clientv3.Cmp{clientv3.Compare(clientv3.Version("k"), "=", 1), clientv3.Compare(clientv3.Version("k"), "<", 1)}}}
...
func TestLeasingTxnNonOwnerPut(t *testing.T) {
	...
	tresp, terr := lkv2.Txn(context.TODO()).Then(clientv3.OpTxn(nil, opArray, nil), clientv3.OpPut("k", "def"), clientv3.OpPut("k3", "999")).Commit()
	if terr != nil {
		t.Fatal(terr)
	}
	...
	w := clus.Client(0).Watch(clus.Client(0).Ctx(), "k", clientv3.WithRev(tresp.Header.Revision), clientv3.WithPrefix())
	wresp := <-w
	c := 0
	evs := []clientv3.Event{}
...
func testLeasingOwnerDelete(t *testing.T, del clientv3.Op) {
	...
	w := clus.Client(0).Watch(clus.Client(0).Ctx(), "key/", clientv3.WithRev(delResp.Header.Revision), clientv3.WithPrefix())
...
func TestLeasingReconnectOwnerConsistency(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		case 2:
			txn := lkv.Txn(context.TODO()).Then(clientv3.OpGet("k"), clientv3.OpDelete("k"))
			_, err = txn.Commit()
		case 3:
			txn := lkv.Txn(context.TODO()).Then(clientv3.OpGet("k"), clientv3.OpPut("k", v))
			_, err = txn.Commit()
		case 4:
...
func TestLeasingReconnectTxn(t *testing.T) {
	...
	_, lerr := lkv.Txn(context.TODO()).If(clientv3.Compare(clientv3.Version("k"), "=", 0)).Then(clientv3.OpGet("k")).Commit()
	<-donec
	if lerr != nil {
		t.Fatal(lerr)
	}
...
func TestLeasingDo(t *testing.T) {
	...
	testutil.AssertNil(t, err)
	defer closeLKV()
	ops := []clientv3.Op{clientv3.OpTxn(nil, nil, nil), clientv3.OpGet("a"), clientv3.OpPut("a/abc", "v"), clientv3.OpDelete("a", clientv3.WithPrefix()), clientv3.OpTxn(nil, nil, nil)}
	for i, op := range ops {
...
func TestLeasingSessionExpireCancel(t *testing.T) {
	...
	clus := integration.NewClusterV3(t, &integration.ClusterConfig{Size: 3})
	defer clus.Terminate(t)
	tests := []func(context.Context, clientv3.KV) error{func(ctx context.Context, kv clientv3.KV) error {
