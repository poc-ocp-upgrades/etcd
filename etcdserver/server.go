...
func NewServer(cfg ServerConfig) (srv *EtcdServer, err error) {
	...
	srv = &EtcdServer{readych: make(chan struct{}), Cfg: cfg, errorc: make(chan error, 1), store: st, snapshotter: ss, r: *newRaftNode(raftNodeConfig{isIDRemoved: func(id uint64) bool {
	...
	tp, err := auth.NewTokenProvider(cfg.AuthToken, func(index uint64) <-chan struct{} {
	...
	tr := &rafthttp.Transport{TLSInfo: cfg.PeerTLSInfo, DialTimeout: cfg.peerDialTimeout(), ID: id, URLs: cfg.PeerURLs, ClusterID: cl.ID(), Raft: srv, Snapshotter: ss, ServerStats: sstats, LeaderStats: lstats, ErrorC: srv.errorc}
...
func (s *EtcdServer) Start() {
	...
	s.goAttach(func() {
		monitorFileDescriptor(s.stopping)
	})
...
func (s *EtcdServer) run() {
	...
	rh := &raftReadyHandler{updateLeadership: func(newLeader bool) {
		if !s.isLeader() {
			if s.lessor != nil {
				s.lessor.Demote()
			}
