...
func (s *EtcdServer) CheckInitialHashKV() error {
	...
	lg := s.getLogger()
	if lg != nil {
		lg.Info("starting initial corruption check", zap.String("local-member-id", s.ID().String()), zap.Duration("timeout", s.Cfg.ReqTimeout()))
	} else {
	...
	for _, p := range peers {
		...
		if p.err != nil {
			...
			case rpctypes.ErrFutureRev:
				if lg != nil {
					lg.Warn("cannot fetch hash from slow remote peer", zap.String("local-member-id", s.ID().String()), zap.Int64("local-member-revision", rev), zap.Int64("local-member-compact-revision", crev), zap.Uint32("local-member-hash", h), zap.String("remote-peer-id", p.id.String()), zap.Strings("remote-peer-endpoints", p.eps), zap.Error(err))
				} else {
			...
			case rpctypes.ErrCompacted:
				if lg != nil {
					lg.Warn("cannot fetch hash from remote peer; local member is behind", zap.String("local-member-id", s.ID().String()), zap.Int64("local-member-revision", rev), zap.Int64("local-member-compact-revision", crev), zap.Uint32("local-member-hash", h), zap.String("remote-peer-id", p.id.String()), zap.Strings("remote-peer-endpoints", p.eps), zap.Error(err))
				} else {
	...
	if lg != nil {
		lg.Info("initial corruption checking passed; no corruption", zap.String("local-member-id", s.ID().String()))
	} else {
...
func (s *EtcdServer) monitorKVHash() {
	...
	lg := s.getLogger()
	if lg != nil {
		lg.Info("enabled corruption checking", zap.String("local-member-id", s.ID().String()), zap.Duration("interval", t))
	} else {
...
func (s *EtcdServer) checkHashKV() error {
	...
	mismatch := func(id uint64) {
		...
		alarmed = true
		a := &pb.AlarmRequest{MemberID: id, Action: pb.AlarmRequest_ACTIVATE, Alarm: pb.AlarmType_CORRUPT}
		s.goAttach(func() {
	...
	if h2 != h && rev2 == rev && crev == crev2 {
		if lg != nil {
			lg.Warn("found hash mismatch", zap.Int64("revision-1", rev), zap.Int64("compact-revision-1", crev), zap.Uint32("hash-1", h), zap.Int64("revision-2", rev2), zap.Int64("compact-revision-2", crev2), zap.Uint32("hash-2", h2))
		} else {
	...
	for _, p := range peers {
		...
		if p.resp.Header.Revision > rev2 {
			if lg != nil {
				lg.Warn("revision from follower must be less than or equal to leader's", zap.Int64("leader-revision", rev2), zap.Int64("follower-revision", p.resp.Header.Revision), zap.String("follower-peer-id", types.ID(id).String()))
			} else {
		...
		if p.resp.CompactRevision > crev2 {
			if lg != nil {
				lg.Warn("compact revision from follower must be less than or equal to leader's", zap.Int64("leader-compact-revision", crev2), zap.Int64("follower-compact-revision", p.resp.CompactRevision), zap.String("follower-peer-id", types.ID(id).String()))
			} else {
		...
		if p.resp.CompactRevision == crev && p.resp.Hash != h {
			if lg != nil {
				lg.Warn("same compact revision then hashes must match", zap.Int64("leader-compact-revision", crev2), zap.Uint32("leader-hash", h), zap.Int64("follower-compact-revision", p.resp.CompactRevision), zap.Uint32("follower-hash", p.resp.Hash), zap.String("follower-peer-id", types.ID(id).String()))
			} else {
...
func (s *EtcdServer) getPeerHashKVs(rev int64) (resps []*peerHashKVResp) {
	...
	for _, p := range pss {
		...
		cli, cerr := clientv3.New(clientv3.Config{DialTimeout: s.Cfg.ReqTimeout(), Endpoints: p.eps})
		if cerr != nil {
			if lg != nil {
				lg.Warn("failed to create client to peer URL", zap.String("local-member-id", s.ID().String()), zap.String("remote-peer-id", p.id.String()), zap.Strings("remote-peer-endpoints", p.eps), zap.Error(cerr))
			} else {
		...
		for _, c := range cli.Endpoints() {
			...
			if lg != nil {
				lg.Warn("failed hash kv request", zap.String("local-member-id", s.ID().String()), zap.Int64("requested-revision", rev), zap.String("remote-peer-endpoint", c), zap.Error(cerr))
			} else {
