...
func userFromClientCertificate(lg *zap.Logger, sec v2auth.Store, r *http.Request) *v2auth.User {
	...
	for _, chains := range r.TLS.VerifiedChains {
		for _, chain := range chains {
			...
			if err == nil {
				if lg != nil {
					lg.Debug("authenticated a user via common name", zap.String("user-name", user.User), zap.String("common-name", chain.Subject.CommonName))
				} else {
...
func hasRootAccess(lg *zap.Logger, sec v2auth.Store, r *http.Request, clientCertAuthEnabled bool) bool {
	...
	if lg != nil {
		lg.Warn("a user does not have root role for resource", zap.String("root-user", rootUser.User), zap.String("root-role-name", v2auth.RootRoleName), zap.String("resource-path", r.URL.Path))
	} else {
...
func hasKeyPrefixAccess(lg *zap.Logger, sec v2auth.Store, r *http.Request, key string, recursive, clientCertAuthEnabled bool) bool {
	...
	if lg != nil {
		lg.Warn("invalid access for user on key", zap.String("user-name", user.User), zap.String("key", key))
	} else {
...
func hasGuestAccess(lg *zap.Logger, sec v2auth.Store, r *http.Request, key string) bool {
	...
	if lg != nil {
		lg.Warn("invalid access for a guest role on key", zap.String("role-name", v2auth.GuestRoleName), zap.String("key", key))
	} else {
...
func writeNoAuth(lg *zap.Logger, w http.ResponseWriter, r *http.Request) {
	...
	if err := herr.WriteTo(w); err != nil {
		if lg != nil {
			lg.Debug("failed to write v2 HTTP error", zap.String("remote-addr", r.RemoteAddr), zap.Error(err))
		} else {
...
func (sh *authHandler) baseRoles(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if sh.lg != nil {
			sh.lg.Warn("failed to encode base roles", zap.String("url", r.URL.String()), zap.Error(err))
		} else {
...
func (sh *authHandler) forRole(w http.ResponseWriter, r *http.Request, role string) {
	...
	case "GET":
		...
		if err != nil {
			if sh.lg != nil {
				sh.lg.Warn("failed to encode a role", zap.String("url", r.URL.String()), zap.Error(err))
			} else {
...
func (sh *authHandler) baseUsers(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if sh.lg != nil {
			sh.lg.Warn("failed to encode users", zap.String("url", r.URL.String()), zap.Error(err))
		} else {
...
func (sh *authHandler) forUser(w http.ResponseWriter, r *http.Request, user string) {
	...
	case "GET":
		...
		if err != nil {
			if sh.lg != nil {
				sh.lg.Warn("failed to encode roles", zap.String("url", r.URL.String()), zap.Error(err))
			} else {
	...
	case "PUT":
		...
		if err != nil {
			if sh.lg != nil {
				sh.lg.Warn("failed to encode a user", zap.String("url", r.URL.String()), zap.Error(err))
			} else {
				plog.Warningf("forUser error encoding on %s", r.URL)
			}
			return
...
func (sh *authHandler) enableDisable(w http.ResponseWriter, r *http.Request) {
	...
	case "GET":
		...
		if err != nil {
			if sh.lg != nil {
				sh.lg.Warn("failed to encode a auth state", zap.String("url", r.URL.String()), zap.Error(err))
			} else {
