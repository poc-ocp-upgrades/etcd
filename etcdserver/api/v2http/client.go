...
func handleV2(lg *zap.Logger, mux *http.ServeMux, server etcdserver.ServerV2, timeout time.Duration) {
	...
	kh := &keysHandler{lg: lg, sec: sec, server: server, cluster: server.Cluster(), timeout: timeout, clientCertAuthEnabled: server.ClientCertAuthEnabled()}
	sh := &statsHandler{lg: lg, stats: server}
	mh := &membersHandler{lg: lg, sec: sec, server: server, cluster: server.Cluster(), timeout: timeout, clock: clockwork.NewRealClock(), clientCertAuthEnabled: server.ClientCertAuthEnabled()}
	mah := &machinesHandler{cluster: server.Cluster()}
	sech := &authHandler{lg: lg, sec: sec, cluster: server.Cluster(), clientCertAuthEnabled: server.ClientCertAuthEnabled()}
...
func (h *membersHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		case err != nil:
			if h.lg != nil {
				h.lg.Warn("failed to add a member", zap.String("member-id", m.ID.String()), zap.Error(err))
			} else {
	...
	case "DELETE":
		...
		case err != nil:
			if h.lg != nil {
				h.lg.Warn("failed to remove a member", zap.String("member-id", id.String()), zap.Error(err))
			} else {
	...
	case "PUT":
		...
		case err != nil:
			if h.lg != nil {
				h.lg.Warn("failed to update a member", zap.String("member-id", m.ID.String()), zap.Error(err))
			} else {
...
func parseKeyRequest(r *http.Request, clock clockwork.Clock) (etcdserverpb.Request, bool, error) {
	...
	err := r.ParseForm()
	if err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidForm, err.Error())
	}
	if !strings.HasPrefix(r.URL.Path, keysPrefix) {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidForm, "incorrect key prefix")
	}
	p := path.Join(etcdserver.StoreKeysPrefix, r.URL.Path[len(keysPrefix):])
	var pIdx, wIdx uint64
	if pIdx, err = getUint64(r.Form, "prevIndex"); err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeIndexNaN, `invalid value for "prevIndex"`)
	}
	if wIdx, err = getUint64(r.Form, "waitIndex"); err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeIndexNaN, `invalid value for "waitIndex"`)
	}
	var rec, sort, wait, dir, quorum, stream bool
	if rec, err = getBool(r.Form, "recursive"); err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, `invalid value for "recursive"`)
	}
	if sort, err = getBool(r.Form, "sorted"); err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, `invalid value for "sorted"`)
	}
	if wait, err = getBool(r.Form, "wait"); err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, `invalid value for "wait"`)
	}
	if dir, err = getBool(r.Form, "dir"); err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, `invalid value for "dir"`)
	}
	if quorum, err = getBool(r.Form, "quorum"); err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, `invalid value for "quorum"`)
	}
	if stream, err = getBool(r.Form, "stream"); err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, `invalid value for "stream"`)
	}
	if wait && r.Method != "GET" {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, `"wait" can only be used with GET requests`)
	}
	pV := r.FormValue("prevValue")
	if _, ok := r.Form["prevValue"]; ok && pV == "" {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodePrevValueRequired, `"prevValue" cannot be empty`)
	}
	if noValueOnSuccess, err = getBool(r.Form, "noValueOnSuccess"); err != nil {
		return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, `invalid value for "noValueOnSuccess"`)
	}
	var ttl *uint64
	if len(r.FormValue("ttl")) > 0 {
		i, err := getUint64(r.Form, "ttl")
		if err != nil {
			return emptyReq, false, v2error.NewRequestError(v2error.EcodeTTLNaN, `invalid value for "ttl"`)
		}
		ttl = &i
	}
	var pe *bool
	if _, ok := r.Form["prevExist"]; ok {
		...
		if err != nil {
			return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, "invalid value for prevExist")
		}
		pe = &bv
	}
	var refresh *bool
	if _, ok := r.Form["refresh"]; ok {
		...
		if err != nil {
			return emptyReq, false, v2error.NewRequestError(v2error.EcodeInvalidField, "invalid value for refresh")
		}
		refresh = &bv
		if refresh != nil && *refresh {
			val := r.FormValue("value")
			if _, ok := r.Form["value"]; ok && val != "" {
				return emptyReq, false, v2error.NewRequestError(v2error.EcodeRefreshValue, `A value was provided on a refresh`)
			}
			if ttl == nil {
				return emptyReq, false, v2error.NewRequestError(v2error.EcodeRefreshTTLRequired, `No TTL value set`)
			}
		}
	}
...
func writeKeyEvent(w http.ResponseWriter, resp etcdserver.Response, noValueOnSuccess bool) error {
	...
	if noValueOnSuccess && (ev.Action == v2store.Set || ev.Action == v2store.CompareAndSwap || ev.Action == v2store.Create || ev.Action == v2store.Update) {
		ev.Node = nil
		ev.PrevNode = nil
	}
...
func writeKeyError(lg *zap.Logger, w http.ResponseWriter, err error) {
	...
	default:
		...
		case etcdserver.ErrTimeoutDueToLeaderFail, etcdserver.ErrTimeoutDueToConnectionLost:
			if lg != nil {
				lg.Warn("v2 response error", zap.String("internal-server-error", err.Error()))
			} else {
				mlog.MergeError(err)
			}
		default:
			if lg != nil {
				lg.Warn("unexpected v2 response error", zap.String("internal-server-error", err.Error()))
			} else {
