...
func TestAuthFlow(t *testing.T) {
	...
	var testCases = []struct {
		...
		wcode	int
		wbody	string
	}{{req: mustJSONRequest(t, "PUT", "users/alice", `{{{{{{{`), store: mockAuthStore{}, wcode: http.StatusBadRequest, wbody: `{"message":"Invalid JSON in request body."}`}, {req: mustJSONRequest(t, "PUT", "users/alice", `{"user": "alice", "password": "goodpassword"}`), store: mockAuthStore{enabled: true}, wcode: http.StatusUnauthorized, wbody: `{"message":"Insufficient credentials"}`}, {req: mustJSONRequest(t, "GET", "users", ""), store: mockAuthStore{users: map[string]*auth.User{"alice": {User: "alice", Roles: []string{"alicerole", "guest"}, Password: "wheeee"}, "bob": {User: "bob", Roles: []string{"guest"}, Password: "wheeee"}, "root": {User: "root", Roles: []string{"root"}, Password: "wheeee"}}, roles: map[string]*auth.Role{"alicerole": {Role: "alicerole"}, "guest": {Role: "guest"}, "root": {Role: "root"}}}, wcode: http.StatusOK, wbody: `{"users":[` + `{"user":"alice","roles":[` + `{"role":"alicerole","permissions":{"kv":{"read":null,"write":null}}},` + `{"role":"guest","permissions":{"kv":{"read":null,"write":null}}}` + `]},` + `{"user":"bob","roles":[{"role":"guest","permissions":{"kv":{"read":null,"write":null}}}]},` + `{"user":"root","roles":[{"role":"root","permissions":{"kv":{"read":null,"write":null}}}]}]}`}, {req: mustJSONRequest(t, "GET", "users/alice", ""), store: mockAuthStore{users: map[string]*auth.User{"alice": {User: "alice", Roles: []string{"alicerole"}, Password: "wheeee"}}, roles: map[string]*auth.Role{"alicerole": {Role: "alicerole"}}}, wcode: http.StatusOK, wbody: `{"user":"alice","roles":[{"role":"alicerole","permissions":{"kv":{"read":null,"write":null}}}]}`}, {req: mustJSONRequest(t, "PUT", "users/alice", `{"user": "alice", "password": "goodpassword"}`), store: mockAuthStore{}, wcode: http.StatusCreated, wbody: `{"user":"alice","roles":null}`}, {req: mustJSONRequest(t, "DELETE", "users/alice", ``), store: mockAuthStore{}, wcode: http.StatusOK, wbody: ``}, {req: mustJSONRequest(t, "PUT", "users/alice", `{"user": "alice", "password": "goodpassword"}`), store: mockAuthStore{users: map[string]*auth.User{"alice": {User: "alice", Roles: []string{"alicerole", "guest"}, Password: "wheeee"}}}, wcode: http.StatusOK, wbody: `{"user":"alice","roles":["alicerole","guest"]}`}, {req: mustJSONRequest(t, "PUT", "users/alice", `{"user": "alice", "grant": ["alicerole"]}`), store: mockAuthStore{users: map[string]*auth.User{"alice": {User: "alice", Roles: []string{"alicerole", "guest"}, Password: "wheeee"}}}, wcode: http.StatusOK, wbody: `{"user":"alice","roles":["alicerole","guest"]}`}, {req: mustJSONRequest(t, "GET", "users/alice", ``), store: mockAuthStore{users: map[string]*auth.User{}, err: auth.Error{Status: http.StatusNotFound, Errmsg: "auth: User alice doesn't exist."}}, wcode: http.StatusNotFound, wbody: `{"message":"auth: User alice doesn't exist."}`}, {req: mustJSONRequest(t, "GET", "roles/manager", ""), store: mockAuthStore{roles: map[string]*auth.Role{"manager": {Role: "manager"}}}, wcode: http.StatusOK, wbody: `{"role":"manager","permissions":{"kv":{"read":null,"write":null}}}`}, {req: mustJSONRequest(t, "DELETE", "roles/manager", ``), store: mockAuthStore{}, wcode: http.StatusOK, wbody: ``}, {req: mustJSONRequest(t, "PUT", "roles/manager", `{"role":"manager","permissions":{"kv":{"read":[],"write":[]}}}`), store: mockAuthStore{}, wcode: http.StatusCreated, wbody: `{"role":"manager","permissions":{"kv":{"read":[],"write":[]}}}`}, {req: mustJSONRequest(t, "PUT", "roles/manager", `{"role":"manager","revoke":{"kv":{"read":["foo"],"write":[]}}}`), store: mockAuthStore{roles: map[string]*auth.Role{"manager": {Role: "manager"}}}, wcode: http.StatusOK, wbody: `{"role":"manager","permissions":{"kv":{"read":null,"write":null}}}`}, {req: mustJSONRequest(t, "GET", "roles", ""), store: mockAuthStore{roles: map[string]*auth.Role{"awesome": {Role: "awesome"}, "guest": {Role: "guest"}, "root": {Role: "root"}}}, wcode: http.StatusOK, wbody: `{"roles":[{"role":"awesome","permissions":{"kv":{"read":null,"write":null}}},` + `{"role":"guest","permissions":{"kv":{"read":null,"write":null}}},` + `{"role":"root","permissions":{"kv":{"read":null,"write":null}}}]}`}, {req: mustJSONRequest(t, "GET", "enable", ""), store: mockAuthStore{enabled: true}, wcode: http.StatusOK, wbody: `{"enabled":true}`}, {req: mustJSONRequest(t, "PUT", "enable", ""), store: mockAuthStore{enabled: false}, wcode: http.StatusOK, wbody: ``}, {req: (func() *http.Request {
		req := mustJSONRequest(t, "DELETE", "enable", "")
		req.SetBasicAuth("root", "good")
		return req
	})(), store: mockAuthStore{enabled: true, users: map[string]*auth.User{"root": {User: "root", Password: goodPassword, Roles: []string{"root"}}}, roles: map[string]*auth.Role{"root": {Role: "root"}}}, wcode: http.StatusOK, wbody: ``}, {req: (func() *http.Request {
		req := mustJSONRequest(t, "DELETE", "enable", "")
		req.SetBasicAuth("root", "bad")
		return req
	})(), store: mockAuthStore{enabled: true, users: map[string]*auth.User{"root": {User: "root", Password: goodPassword, Roles: []string{"root"}}}, roles: map[string]*auth.Role{"root": {Role: "guest"}}}, wcode: http.StatusUnauthorized, wbody: `{"message":"Insufficient credentials"}`}}
	for i, tt := range testCases {
		mux := http.NewServeMux()
		h := &authHandler{sec: &tt.store, cluster: &fakeCluster{id: 1}}
		handleAuth(mux, h)
...
func TestPrefixAccess(t *testing.T) {
	...
	}{{key: "/foo", req: mustAuthRequest("GET", "root", "good"), store: &mockAuthStore{users: map[string]*auth.User{"root": {User: "root", Password: goodPassword, Roles: []string{"root"}}}, roles: map[string]*auth.Role{"root": {Role: "root"}}, enabled: true}, hasRoot: true, hasKeyPrefixAccess: true, hasRecursiveAccess: true}, {key: "/foo", req: mustAuthRequest("GET", "user", "good"), store: &mockAuthStore{users: map[string]*auth.User{"user": {User: "user", Password: goodPassword, Roles: []string{"foorole"}}}, roles: map[string]*auth.Role{"foorole": {Role: "foorole", Permissions: auth.Permissions{KV: auth.RWPermission{Read: []string{"/foo"}, Write: []string{"/foo"}}}}}, enabled: true}, hasRoot: false, hasKeyPrefixAccess: true, hasRecursiveAccess: false}, {key: "/foo", req: mustAuthRequest("GET", "user", "good"), store: &mockAuthStore{users: map[string]*auth.User{"user": {User: "user", Password: goodPassword, Roles: []string{"foorole"}}}, roles: map[string]*auth.Role{"foorole": {Role: "foorole", Permissions: auth.Permissions{KV: auth.RWPermission{Read: []string{"/foo*"}, Write: []string{"/foo*"}}}}}, enabled: true}, hasRoot: false, hasKeyPrefixAccess: true, hasRecursiveAccess: true}, {key: "/foo", req: mustAuthRequest("GET", "user", "bad"), store: &mockAuthStore{users: map[string]*auth.User{"user": {User: "user", Password: goodPassword, Roles: []string{"foorole"}}}, roles: map[string]*auth.Role{"foorole": {Role: "foorole", Permissions: auth.Permissions{KV: auth.RWPermission{Read: []string{"/foo*"}, Write: []string{"/foo*"}}}}}, enabled: true}, hasRoot: false, hasKeyPrefixAccess: false, hasRecursiveAccess: false}, {key: "/foo", req: mustAuthRequest("GET", "user", "good"), store: &mockAuthStore{users: map[string]*auth.User{}, err: errors.New("Not the user"), enabled: true}, hasRoot: false, hasKeyPrefixAccess: false, hasRecursiveAccess: false}, {key: "/foo", req: mustJSONRequest(t, "GET", "somepath", ""), store: &mockAuthStore{users: map[string]*auth.User{"user": {User: "user", Password: goodPassword, Roles: []string{"foorole"}}}, roles: map[string]*auth.Role{"guest": {Role: "guest", Permissions: auth.Permissions{KV: auth.RWPermission{Read: []string{"/foo*"}, Write: []string{"/foo*"}}}}}, enabled: true}, hasRoot: false, hasKeyPrefixAccess: true, hasRecursiveAccess: true}, {key: "/bar", req: mustJSONRequest(t, "GET", "somepath", ""), store: &mockAuthStore{users: map[string]*auth.User{"user": {User: "user", Password: goodPassword, Roles: []string{"foorole"}}}, roles: map[string]*auth.Role{"guest": {Role: "guest", Permissions: auth.Permissions{KV: auth.RWPermission{Read: []string{"/foo*"}, Write: []string{"/foo*"}}}}}, enabled: true}, hasRoot: false, hasKeyPrefixAccess: false, hasRecursiveAccess: false}, {key: "/foo", req: mustAuthRequest("GET", "user", "good"), store: &mockAuthStore{users: map[string]*auth.User{"user": {User: "user", Password: goodPassword, Roles: []string{"role1", "role2"}}}, roles: map[string]*auth.Role{"role1": {Role: "role1"}, "role2": {Role: "role2", Permissions: auth.Permissions{KV: auth.RWPermission{Read: []string{"/foo"}, Write: []string{"/foo"}}}}}, enabled: true}, hasRoot: false, hasKeyPrefixAccess: true, hasRecursiveAccess: false}, {key: "/foo", req: (func() *http.Request {
		req := mustJSONRequest(t, "GET", "somepath", "")
		req.Header.Set("Authorization", "malformedencoding")
		return req
	})(), store: &mockAuthStore{enabled: true, users: map[string]*auth.User{"root": {User: "root", Password: goodPassword, Roles: []string{"root"}}}, roles: map[string]*auth.Role{"guest": {Role: "guest", Permissions: auth.Permissions{KV: auth.RWPermission{Read: []string{"/foo*"}, Write: []string{"/foo*"}}}}}}, hasRoot: false, hasKeyPrefixAccess: false, hasRecursiveAccess: false}, {key: "/foo", req: (func() *http.Request {
		return mustJSONRequest(t, "GET", "somepath", "")
	})(), store: &mockAuthStore{enabled: true, users: map[string]*auth.User{"root": {User: "root", Password: goodPassword, Roles: []string{"root"}}}, roles: map[string]*auth.Role{"guest": {Role: "guest", Permissions: auth.Permissions{KV: auth.RWPermission{Read: []string{"/foo*"}, Write: []string{"/foo*"}}}}}}, hasRoot: false, hasKeyPrefixAccess: true, hasRecursiveAccess: true}}
...
func TestUserFromClientCertificate(t *testing.T) {
	...
	var table = []struct {
		...
		store		auth.Store
		username	string
	}{{req: unauthedRequest("GET"), userExists: false, store: witherror}, {req: tlsAuthedRequest(unauthedRequest("GET"), "user"), userExists: true, username: "user", store: noerror}, {req: tlsAuthedRequest(unauthedRequest("GET"), "otheruser"), userExists: false, store: witherror}}
	for i, tt := range table {
...
func TestUserFromBasicAuth(t *testing.T) {
	...
	var table = []struct {
		...
		req		*http.Request
		userExists	bool
	}{{username: "user", req: mustAuthRequest("GET", "user", "password"), userExists: true}, {username: "user", req: mustAuthRequest("GET", "user", "badpass"), userExists: false}, {username: "user", req: mustAuthRequest("GET", "user", ""), userExists: false}, {username: "missing", req: mustAuthRequest("GET", "missing", "badpass"), userExists: false}, {req: unauthedRequest("GET"), userExists: false}}
	for i, tt := range table {
