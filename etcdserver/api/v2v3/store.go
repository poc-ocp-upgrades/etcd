...
func (s *v2v3Store) Get(nodePath string, recursive, sorted bool) (*v2store.Event, error) {
	...
	if kvs := resp.Responses[0].GetResponseRange().Kvs; len(kvs) != 0 || isRoot(nodePath) {
		...
		return &v2store.Event{Action: v2store.Get, Node: &v2store.NodeExtern{Key: nodePath, Dir: true, Nodes: nodes, CreatedIndex: cidx, ModifiedIndex: midx}, EtcdIndex: mkV2Rev(resp.Header.Revision)}, nil
	}
	...
	return &v2store.Event{Action: v2store.Get, Node: s.mkV2Node(kvs[0]), EtcdIndex: mkV2Rev(resp.Header.Revision)}, nil
}
...
func (s *v2v3Store) Set(nodePath string, dir bool, value string, expireOpts v2store.TTLOptionSet) (*v2store.Event, error) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	...
	return &v2store.Event{Action: v2store.Set, Node: &v2store.NodeExtern{Key: nodePath, Value: vp, Dir: dir, ModifiedIndex: mkV2Rev(resp.Header.Revision), CreatedIndex: mkV2Rev(createRev)}, PrevNode: pn, EtcdIndex: mkV2Rev(resp.Header.Revision)}, nil
}
func (s *v2v3Store) Update(nodePath, newValue string, expireOpts v2store.TTLOptionSet) (*v2store.Event, error) {
	...
	return &v2store.Event{Action: v2store.Update, Node: &v2store.NodeExtern{Key: nodePath, Value: &newValue, ModifiedIndex: mkV2Rev(resp.Header.Revision), CreatedIndex: mkV2Rev(pkv.CreateRevision)}, PrevNode: s.mkV2Node(pkv), EtcdIndex: mkV2Rev(resp.Header.Revision)}, nil
}
func (s *v2v3Store) Create(nodePath string, dir bool, value string, unique bool, expireOpts v2store.TTLOptionSet) (*v2store.Event, error) {
	...
	return &v2store.Event{Action: v2store.Create, Node: &v2store.NodeExtern{Key: nodePath, Value: v, Dir: dir, ModifiedIndex: mkV2Rev(resp.Header.Revision), CreatedIndex: mkV2Rev(resp.Header.Revision)}, EtcdIndex: mkV2Rev(resp.Header.Revision)}, nil
}
func (s *v2v3Store) CompareAndSwap(nodePath string, prevValue string, prevIndex uint64, value string, expireOpts v2store.TTLOptionSet) (*v2store.Event, error) {
	...
	key := s.mkPath(nodePath)
	resp, err := s.c.Txn(s.ctx).If(s.mkCompare(nodePath, prevValue, prevIndex)...).Then(clientv3.OpPut(key, value, clientv3.WithPrevKV()), clientv3.OpPut(s.mkActionKey(), v2store.CompareAndSwap)).Else(clientv3.OpGet(key), clientv3.OpGet(key+"/")).Commit()
	if err != nil {
		return nil, err
	}
	if !resp.Succeeded {
	...
	return &v2store.Event{Action: v2store.CompareAndSwap, Node: &v2store.NodeExtern{Key: nodePath, Value: &value, CreatedIndex: mkV2Rev(pkv.CreateRevision), ModifiedIndex: mkV2Rev(resp.Header.Revision)}, PrevNode: s.mkV2Node(pkv), EtcdIndex: mkV2Rev(resp.Header.Revision)}, nil
}
func (s *v2v3Store) Delete(nodePath string, dir, recursive bool) (*v2store.Event, error) {
	...
	return &v2store.Event{Action: v2store.Delete, PrevNode: s.mkV2Node(dresp.PrevKvs[0]), EtcdIndex: mkV2Rev(resp.Header.Revision)}, nil
}
func (s *v2v3Store) deleteEmptyDir(nodePath string) (*v2store.Event, error) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	resp, err := s.c.Txn(s.ctx).If(clientv3.Compare(clientv3.Version(s.mkPathDepth(nodePath, 1)), "=", 0).WithPrefix()).Then(clientv3.OpDelete(s.mkPath(nodePath)+"/", clientv3.WithPrevKV()), clientv3.OpPut(s.mkActionKey(), v2store.Delete)).Commit()
	if err != nil {
		return nil, err
	}
	if !resp.Succeeded {
...
func (s *v2v3Store) deleteNode(nodePath string) (*v2store.Event, error) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	resp, err := s.c.Txn(s.ctx).If(clientv3.Compare(clientv3.Version(s.mkPath(nodePath)+"/"), "=", 0)).Then(clientv3.OpDelete(s.mkPath(nodePath), clientv3.WithPrevKV()), clientv3.OpPut(s.mkActionKey(), v2store.Delete)).Commit()
	if err != nil {
		return nil, err
	}
	if !resp.Succeeded {
	...
	pkv := pkvs[0]
	return &v2store.Event{Action: v2store.Delete, Node: &v2store.NodeExtern{Key: nodePath, CreatedIndex: mkV2Rev(pkv.CreateRevision), ModifiedIndex: mkV2Rev(resp.Header.Revision)}, PrevNode: s.mkV2Node(pkv), EtcdIndex: mkV2Rev(resp.Header.Revision)}, nil
}
func (s *v2v3Store) CompareAndDelete(nodePath, prevValue string, prevIndex uint64) (*v2store.Event, error) {
	...
	key := s.mkPath(nodePath)
	resp, err := s.c.Txn(s.ctx).If(s.mkCompare(nodePath, prevValue, prevIndex)...).Then(clientv3.OpDelete(key, clientv3.WithPrevKV()), clientv3.OpPut(s.mkActionKey(), v2store.CompareAndDelete)).Else(clientv3.OpGet(key), clientv3.OpGet(key+"/")).Commit()
	if err != nil {
		return nil, err
	}
	if !resp.Succeeded {
	...
	return &v2store.Event{Action: v2store.CompareAndDelete, Node: &v2store.NodeExtern{Key: nodePath, CreatedIndex: mkV2Rev(pkv.CreateRevision), ModifiedIndex: mkV2Rev(resp.Header.Revision)}, PrevNode: s.mkV2Node(pkv), EtcdIndex: mkV2Rev(resp.Header.Revision)}, nil
}
