...
func (s *v2v3Store) mkV2Events(wr clientv3.WatchResponse) (evs []*v2store.Event) {
	...
	for _, rev := range mkRevs(wr) {
		...
		v2ev := &v2store.Event{Action: string(act.Kv.Value), Node: s.mkV2Node(key.Kv), PrevNode: s.mkV2Node(key.PrevKv), EtcdIndex: mkV2Rev(wr.Header.Revision)}
		evs = append(evs, v2ev)
	}
	return evs
}
...
func (w *v2v3Watcher) EventChan() chan *v2store.Event {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return w.evc
}
