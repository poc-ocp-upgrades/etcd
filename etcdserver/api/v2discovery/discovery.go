...
func newDiscovery(lg *zap.Logger, durl, dproxyurl string, id types.ID) (*discovery, error) {
	...
	return &discovery{lg: lg, cluster: token, c: dc, id: id, url: u, clock: clockwork.NewRealClock()}, nil
}
...
func (d *discovery) checkCluster() ([]*client.Node, int, uint64, error) {
	...
	if err != nil {
		...
		if ce, ok := err.(*client.ClusterError); ok {
			if d.lg != nil {
				d.lg.Warn("failed to get from discovery server", zap.String("discovery-url", d.url.String()), zap.String("path", path.Join(configKey, "size")), zap.Error(err), zap.String("err-detail", ce.Detail()))
			} else {
	...
	if err != nil {
		if ce, ok := err.(*client.ClusterError); ok {
			if d.lg != nil {
				d.lg.Warn("failed to get from discovery server", zap.String("discovery-url", d.url.String()), zap.String("path", d.cluster), zap.Error(err), zap.String("err-detail", ce.Detail()))
			} else {
				plog.Error(ce.Detail())
			}
			return d.checkClusterRetry()
...
func (d *discovery) logAndBackoffForRetry(step string) {
	...
	if d.lg != nil {
		d.lg.Info("retry connecting to discovery service", zap.String("url", d.url.String()), zap.String("reason", step), zap.Duration("backoff", retryTimeInSecond))
	} else {
...
func (d *discovery) waitNodes(nodes []*client.Node, size int, index uint64) ([]*client.Node, error) {
	...
	for _, n := range all {
		if path.Base(n.Key) == path.Base(d.selfKey()) {
			if d.lg != nil {
				d.lg.Info("found self from discovery server", zap.String("discovery-url", d.url.String()), zap.String("self", path.Base(d.selfKey())))
			} else {
				plog.Noticef("found self %s in the cluster", path.Base(d.selfKey()))
			}
		} else {
			if d.lg != nil {
				d.lg.Info("found peer from discovery server", zap.String("discovery-url", d.url.String()), zap.String("peer", path.Base(n.Key)))
			} else {
	...
	for len(all) < size {
		if d.lg != nil {
			d.lg.Info("found peers from discovery server; waiting for more", zap.String("discovery-url", d.url.String()), zap.Int("found-peers", len(all)), zap.Int("needed-peers", size-len(all)))
		} else {
		...
		if d.lg != nil {
			d.lg.Info("found peer from discovery server", zap.String("discovery-url", d.url.String()), zap.String("peer", path.Base(resp.Node.Key)))
		} else {
	...
	if d.lg != nil {
		d.lg.Info("found all needed peers from discovery server", zap.String("discovery-url", d.url.String()), zap.Int("found-peers", len(all)))
	} else {
