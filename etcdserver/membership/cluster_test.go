...
func TestClusterMemberByName(t *testing.T) {
	...
	tests := []struct {
		name	string
		match	bool
	}{{"node1", true}, {"node2", true}, {"node3", false}}
	for i, tt := range tests {
		c := newTestCluster(membs)
...
func TestClusterPeerURLs(t *testing.T) {
	...
	tests := []struct {
		mems	[]*Member
		wurls	[]string
	}{{mems: []*Member{newTestMember(1, []string{"http://192.0.2.1"}, "", nil)}, wurls: []string{"http://192.0.2.1"}}, {mems: []*Member{newTestMember(1, []string{"http://192.0.2.1:8001"}, "", nil)}, wurls: []string{"http://192.0.2.1:8001"}}, {mems: []*Member{newTestMember(2, []string{"http://192.0.2.3", "http://192.0.2.4"}, "", nil), newTestMember(3, []string{"http://192.0.2.5", "http://192.0.2.6"}, "", nil), newTestMember(1, []string{"http://192.0.2.1", "http://192.0.2.2"}, "", nil)}, wurls: []string{"http://192.0.2.1", "http://192.0.2.2", "http://192.0.2.3", "http://192.0.2.4", "http://192.0.2.5", "http://192.0.2.6"}}, {mems: []*Member{}, wurls: []string{}}, {mems: []*Member{newTestMember(3, []string{}, "", nil)}, wurls: []string{}}}
	for i, tt := range tests {
...
func TestClusterClientURLs(t *testing.T) {
	...
	tests := []struct {
		mems	[]*Member
		wurls	[]string
	}{{mems: []*Member{newTestMember(1, nil, "", []string{"http://192.0.2.1"})}, wurls: []string{"http://192.0.2.1"}}, {mems: []*Member{newTestMember(1, nil, "", []string{"http://192.0.2.1:8001"})}, wurls: []string{"http://192.0.2.1:8001"}}, {mems: []*Member{newTestMember(2, nil, "", []string{"http://192.0.2.3", "http://192.0.2.4"}), newTestMember(3, nil, "", []string{"http://192.0.2.5", "http://192.0.2.6"}), newTestMember(1, nil, "", []string{"http://192.0.2.1", "http://192.0.2.2"})}, wurls: []string{"http://192.0.2.1", "http://192.0.2.2", "http://192.0.2.3", "http://192.0.2.4", "http://192.0.2.5", "http://192.0.2.6"}}, {mems: []*Member{}, wurls: []string{}}, {mems: []*Member{newTestMember(3, nil, "", []string{})}, wurls: []string{}}}
	for i, tt := range tests {
		c := newTestCluster(tt.mems)
		urls := c.ClientURLs()
		if !reflect.DeepEqual(urls, tt.wurls) {
...
func TestClusterValidateAndAssignIDsBad(t *testing.T) {
	...
	tests := []struct {
		clmembs	[]*Member
		membs	[]*Member
	}{{[]*Member{newTestMember(1, []string{"http://127.0.0.1:2379"}, "", nil)}, []*Member{}}, {[]*Member{newTestMember(1, []string{"http://127.0.0.1:2379"}, "", nil)}, []*Member{newTestMember(1, []string{"http://127.0.0.1:4001"}, "", nil)}}, {[]*Member{newTestMember(1, []string{"http://127.0.0.1:2379"}, "", nil), newTestMember(2, []string{"http://127.0.0.2:2379"}, "", nil)}, []*Member{newTestMember(1, []string{"http://127.0.0.1:2379"}, "", nil), newTestMember(2, []string{"http://127.0.0.2:4001"}, "", nil)}}}
	for i, tt := range tests {
...
func TestClusterValidateAndAssignIDs(t *testing.T) {
	...
	tests := []struct {
		...
		membs	[]*Member
		wids	[]types.ID
	}{{[]*Member{newTestMember(1, []string{"http://127.0.0.1:2379"}, "", nil), newTestMember(2, []string{"http://127.0.0.2:2379"}, "", nil)}, []*Member{newTestMember(3, []string{"http://127.0.0.1:2379"}, "", nil), newTestMember(4, []string{"http://127.0.0.2:2379"}, "", nil)}, []types.ID{3, 4}}}
	for i, tt := range tests {
...
func TestClusterAddMember(t *testing.T) {
	...
	wactions := []testutil.Action{{Name: "Create", Params: []interface{}{path.Join(StoreMembersPrefix, "1", "raftAttributes"), false, `{"peerURLs":null}`, false, store.TTLOptionSet{ExpireTime: store.Permanent}}}}
...
func TestClusterRemoveMember(t *testing.T) {
	...
	c.SetStore(st)
	c.RemoveMember(1)
	wactions := []testutil.Action{{Name: "Delete", Params: []interface{}{MemberStoreKey(1), true, true}}, {Name: "Create", Params: []interface{}{RemovedMemberStoreKey(1), false, "", false, store.TTLOptionSet{ExpireTime: store.Permanent}}}}
...
func TestClusterUpdateAttributes(t *testing.T) {
	...
	tests := []struct {
		...
		wmems	[]*Member
	}{{[]*Member{newTestMember(1, nil, "", nil)}, nil, []*Member{newTestMember(1, nil, name, clientURLs)}}, {nil, map[types.ID]bool{types.ID(1): true}, nil}}
	for i, tt := range tests {
		c := newTestCluster(tt.mems)
...
func TestIsReadyToAddNewMember(t *testing.T) {
	...
	tests := []struct {
		members	[]*Member
		want	bool
	}{{[]*Member{newTestMember(1, nil, "", nil), newTestMember(2, nil, "", nil), newTestMember(3, nil, "", nil)}, false}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "", nil)}, false}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "", nil), newTestMember(3, nil, "", nil)}, false}, {[]*Member{newTestMember(1, nil, "1", nil)}, true}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "2", nil), newTestMember(3, nil, "", nil)}, false}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "2", nil), newTestMember(3, nil, "3", nil)}, true}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "2", nil), newTestMember(3, nil, "3", nil), newTestMember(4, nil, "", nil)}, true}, {[]*Member{}, false}}
	for i, tt := range tests {
...
func TestIsReadyToRemoveMember(t *testing.T) {
	...
	tests := []struct {
		...
		removeID	uint64
		want		bool
	}{{[]*Member{newTestMember(1, nil, "1", nil)}, 1, false}, {[]*Member{newTestMember(1, nil, "", nil), newTestMember(2, nil, "", nil), newTestMember(3, nil, "", nil)}, 1, false}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "", nil)}, 2, true}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "2", nil), newTestMember(3, nil, "", nil)}, 2, false}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "2", nil), newTestMember(3, nil, "3", nil)}, 3, true}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "2", nil), newTestMember(3, nil, "3", nil), newTestMember(4, nil, "", nil)}, 3, true}, {[]*Member{newTestMember(1, nil, "1", nil), newTestMember(2, nil, "2", nil), newTestMember(3, nil, "3", nil), newTestMember(4, nil, "", nil)}, 4, true}}
	for i, tt := range tests {
		c := newTestCluster(tt.members)
