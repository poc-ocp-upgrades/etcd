...
func newRunnerStresser(stype rpcpb.StresserType, ep string, lg *zap.Logger, cmdStr string, args []string, rl *rate.Limiter, reqRate int) *runnerStresser {
	_logClusterCodePath()
	defer _logClusterCodePath()
	rl.SetLimit(rl.Limit() - rate.Limit(reqRate))
	return &runnerStresser{stype: stype, etcdClientEndpoint: ep, lg: lg, cmdStr: cmdStr, args: args, rl: rl, reqRate: reqRate, errc: make(chan error, 1), donec: make(chan struct{})}
}
