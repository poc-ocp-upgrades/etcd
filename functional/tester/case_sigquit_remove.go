...
func inject_SIGQUIT_ETCD_AND_REMOVE_DATA(clus *Cluster, idx1 int) error {
	...
	clus.lg.Info("member list before disastrous machine failure", zap.String("request-to", clus.Members[idx1].EtcdClientEndpoint), zap.Strings("members", mss), zap.Error(err))
	if err != nil {
		return err
	}
	...
	clus.lg.Info("disastrous machine failure START", zap.String("target-endpoint", clus.Members[idx1].EtcdClientEndpoint), zap.String("target-member-id", is1), zap.Error(err))
	err = clus.sendOp(idx1, rpcpb.Operation_SIGQUIT_ETCD_AND_REMOVE_DATA)
	clus.lg.Info("disastrous machine failure END", zap.String("target-endpoint", clus.Members[idx1].EtcdClientEndpoint), zap.String("target-member-id", is1), zap.Error(err))
	if err != nil {
		return err
	}
	...
	defer cli2.Close()
	clus.lg.Info("member remove after disaster START", zap.String("target-endpoint", clus.Members[idx1].EtcdClientEndpoint), zap.String("target-member-id", is1), zap.String("request-to", clus.Members[idx2].EtcdClientEndpoint))
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	_, err = cli2.MemberRemove(ctx, id1)
	cancel()
	clus.lg.Info("member remove after disaster END", zap.String("target-endpoint", clus.Members[idx1].EtcdClientEndpoint), zap.String("target-member-id", is1), zap.String("request-to", clus.Members[idx2].EtcdClientEndpoint), zap.Error(err))
	if err != nil {
		return err
	}
	time.Sleep(2 * time.Second)
	...
	clus.lg.Info("member list after member remove", zap.String("request-to", clus.Members[idx2].EtcdClientEndpoint), zap.Strings("members", mss), zap.Error(err))
	return err
}
func recover_SIGQUIT_ETCD_AND_REMOVE_DATA(clus *Cluster, idx1 int) error {
	...
	clus.lg.Info("member add before fresh restart", zap.String("target-endpoint", clus.Members[idx1].EtcdClientEndpoint), zap.String("request-to", clus.Members[idx2].EtcdClientEndpoint), zap.Error(err))
	if err != nil {
		return err
	}
	time.Sleep(2 * time.Second)
	...
	clus.lg.Info("fresh restart after member add", zap.String("target-endpoint", clus.Members[idx1].EtcdClientEndpoint), zap.Error(err))
	if err != nil {
		return err
	}
	time.Sleep(2 * time.Second)
	...
	clus.lg.Info("member list after member add", zap.String("request-to", clus.Members[idx2].EtcdClientEndpoint), zap.Strings("members", mss), zap.Error(err))
	return err
}
...
func describeMembers(mresp *clientv3.MemberListResponse) (ss []string) {
	...
	for i, m := range mresp.Members {
		ss[i] = fmt.Sprintf("Name %s / ID %016x / ClientURLs %s / PeerURLs %s", m.Name, m.ID, strings.Join(m.ClientURLs, ","), strings.Join(m.PeerURLs, ","))
	}
	sort.Strings(ss)
	return ss
}
