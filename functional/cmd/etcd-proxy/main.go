...
$ ./bin/etcdctl --endpoints localhost:23790 put foo bar`)
	...
	flag.Parse()
	cfg := proxy.ServerConfig{From: url.URL{Scheme: "tcp", Host: from}, To: url.URL{Scheme: "tcp", Host: to}}
	if verbose {
	...
	srv := &http.Server{Addr: fmt.Sprintf(":%d", httpPort), Handler: mux, ErrorLog: log.New(ioutil.Discard, "net/http", 0)}
	defer srv.Close()
