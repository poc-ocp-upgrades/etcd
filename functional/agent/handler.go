...
func (srv *Server) creatEtcd(fromSnapshot bool) error {
	...
	if srv.Member.EtcdExec != "embed" {
		...
		srv.lg.Info("creating etcd command", zap.String("etcd-exec", etcdPath), zap.Strings("etcd-flags", etcdFlags), zap.String("failpoint-http-addr", srv.Member.FailpointHTTPAddr), zap.String("failpoint-addr", u.Host))
...
func (srv *Server) runEtcd() error {
	...
	if srv.etcdCmd != nil {
		srv.lg.Info("starting etcd command", zap.String("command-path", srv.etcdCmd.Path))
...
func (srv *Server) stopEtcd(sig os.Signal) error {
	...
	srv.stopProxy()
	if srv.etcdCmd != nil {
		srv.lg.Info("stopping etcd command", zap.String("command-path", srv.etcdCmd.Path), zap.String("signal", sig.String()))
...
func (srv *Server) startProxy() error {
	...
	if srv.Member.EtcdClientProxy {
		...
		srv.advertiseClientPortToProxy[advertiseClientURLPort] = proxy.NewServer(proxy.ServerConfig{Logger: srv.lg, From: *advertiseClientURL, To: *listenClientURL})
		select {
	...
	if srv.Member.EtcdPeerProxy {
		...
		srv.advertisePeerPortToProxy[advertisePeerURLPort] = proxy.NewServer(proxy.ServerConfig{Logger: srv.lg, From: *advertisePeerURL, To: *listenPeerURL})
		select {
...
func (srv *Server) stopProxy() {
	...
	if srv.Member.EtcdClientProxy && len(srv.advertiseClientPortToProxy) > 0 {
		for port, px := range srv.advertiseClientPortToProxy {
			...
			srv.lg.Info("closed proxy", zap.Int("port", port), zap.String("from", px.From()), zap.String("to", px.To()))
		}
...
func (srv *Server) saveTLSAssets() error {
	...
	if srv.Member.PeerCertPath != "" && srv.Member.PeerKeyPath != "" && srv.Member.PeerTrustedCAPath != "" {
		srv.lg.Info("wrote", zap.String("peer-cert", srv.Member.PeerCertPath), zap.String("peer-key", srv.Member.PeerKeyPath), zap.String("peer-trusted-ca", srv.Member.PeerTrustedCAPath))
	}
	...
	if srv.Member.ClientCertPath != "" && srv.Member.ClientKeyPath != "" && srv.Member.ClientTrustedCAPath != "" {
		srv.lg.Info("wrote", zap.String("client-cert", srv.Member.ClientCertPath), zap.String("client-key", srv.Member.ClientKeyPath), zap.String("client-trusted-ca", srv.Member.ClientTrustedCAPath))
	}
	return nil
}
func (srv *Server) loadAutoTLSAssets() error {
	...
	if srv.Member.Etcd.PeerAutoTLS {
		...
		srv.lg.Info("loading client auto TLS assets", zap.String("dir", fdir), zap.String("endpoint", srv.EtcdClientEndpoint))
		...
		srv.lg.Info("loaded peer auto TLS assets", zap.String("peer-cert-path", certPath), zap.Int("peer-cert-length", len(certData)), zap.String("peer-key-path", keyPath), zap.Int("peer-key-length", len(keyData)))
	}
	if srv.Member.Etcd.ClientAutoTLS {
		time.Sleep(time.Second)
		fdir := filepath.Join(srv.Member.Etcd.DataDir, "fixtures", "client")
		srv.lg.Info("loading client TLS assets", zap.String("dir", fdir), zap.String("endpoint", srv.EtcdClientEndpoint))
		certPath := filepath.Join(fdir, "cert.pem")
		if !fileutil.Exist(certPath) {
			return fmt.Errorf("cannot find %q", certPath)
		}
		...
		srv.lg.Info("loaded client TLS assets", zap.String("peer-cert-path", certPath), zap.Int("peer-cert-length", len(certData)), zap.String("peer-key-path", keyPath), zap.Int("peer-key-length", len(keyData)))
	}
	return nil
}
...
func (srv *Server) handle_RESTART_ETCD() (*rpcpb.Response, error) {
	...
	return &rpcpb.Response{Success: true, Status: "restart etcd PASS", Member: srv.Member}, nil
}
...
func (srv *Server) handle_SIGQUIT_ETCD_AND_REMOVE_DATA() (*rpcpb.Response, error) {
	...
	srv.lg.Info("renamed", zap.String("base-dir", srv.Member.BaseDir), zap.String("new-dir", srv.Member.BaseDir+".backup"))
	if !fileutil.Exist(srv.Member.BaseDir) {
		err = fileutil.TouchDirAll(srv.Member.BaseDir)
		if err != nil {
			return nil, err
	...
	return &rpcpb.Response{Success: true, Status: "killed etcd and removed base directory"}, nil
}
...
func (srv *Server) handle_SIGQUIT_ETCD_AND_ARCHIVE_DATA() (*rpcpb.Response, error) {
	...
	if err = archive(srv.Member.BaseDir, srv.Member.Etcd.LogOutputs[0], srv.Member.Etcd.DataDir); err != nil {
		return nil, err
	}
	...
	return &rpcpb.Response{Success: true, Status: "cleaned up etcd"}, nil
}
...
func (srv *Server) handle_BLACKHOLE_PEER_PORT_TX_RX() *rpcpb.Response {
	...
	return &rpcpb.Response{Success: true, Status: "blackholed peer port tx/rx"}
}
func (srv *Server) handle_UNBLACKHOLE_PEER_PORT_TX_RX() *rpcpb.Response {
	...
	return &rpcpb.Response{Success: true, Status: "unblackholed peer port tx/rx"}
}
func (srv *Server) handle_DELAY_PEER_PORT_TX_RX() *rpcpb.Response {
	...
	for port, px := range srv.advertisePeerPortToProxy {
		srv.lg.Info("delaying", zap.Int("peer-port", port), zap.Duration("latency", lat), zap.Duration("random-variable", rv))
		px.DelayTx(lat, rv)
		px.DelayRx(lat, rv)
		srv.lg.Info("delayed", zap.Int("peer-port", port), zap.Duration("latency", lat), zap.Duration("random-variable", rv))
	}
	return &rpcpb.Response{Success: true, Status: "delayed peer port tx/rx"}
}
func (srv *Server) handle_UNDELAY_PEER_PORT_TX_RX() *rpcpb.Response {
	...
	return &rpcpb.Response{Success: true, Status: "undelayed peer port tx/rx"}
}
