...
var (
	configNoTLS		= etcdProcessClusterConfig{clusterSize: 3, initialToken: "new"}
	configAutoTLS		= etcdProcessClusterConfig{clusterSize: 3, isPeerTLS: true, isPeerAutoTLS: true, initialToken: "new"}
	configTLS		= etcdProcessClusterConfig{clusterSize: 3, clientTLS: clientTLS, isPeerTLS: true, initialToken: "new"}
	configClientTLS		= etcdProcessClusterConfig{clusterSize: 3, clientTLS: clientTLS, initialToken: "new"}
	configClientBoth	= etcdProcessClusterConfig{clusterSize: 1, clientTLS: clientTLSAndNonTLS, initialToken: "new"}
	configClientAutoTLS	= etcdProcessClusterConfig{clusterSize: 1, isClientAutoTLS: true, clientTLS: clientTLS, initialToken: "new"}
	configPeerTLS		= etcdProcessClusterConfig{clusterSize: 3, isPeerTLS: true, initialToken: "new"}
	configClientTLSCertAuth	= etcdProcessClusterConfig{clusterSize: 1, clientTLS: clientTLS, initialToken: "new", clientCertAuthEnabled: true}
)
...
func (cfg *etcdProcessClusterConfig) etcdServerProcessConfigs() []*etcdServerProcessConfig {
	...
	for i := 0; i < cfg.clusterSize; i++ {
		...
		args := []string{"--name", name, "--listen-client-urls", strings.Join(curls, ","), "--advertise-client-urls", strings.Join(curls, ","), "--listen-peer-urls", purl.String(), "--initial-advertise-peer-urls", purl.String(), "--initial-cluster-token", cfg.initialToken, "--data-dir", dataDirPath, "--snapshot-count", fmt.Sprintf("%d", cfg.snapCount)}
		args = addV2Args(args)
		if cfg.forceNewCluster {
			args = append(args, "--force-new-cluster")
		}
		if cfg.quotaBackendBytes > 0 {
			args = append(args, "--quota-backend-bytes", fmt.Sprintf("%d", cfg.quotaBackendBytes))
		}
		...
		if cfg.metricsURLScheme != "" {
			murl = (&url.URL{Scheme: cfg.metricsURLScheme, Host: fmt.Sprintf("localhost:%d", port+2)}).String()
			args = append(args, "--listen-metrics-urls", murl)
		}
		args = append(args, cfg.tlsArgs()...)
		etcdCfgs[i] = &etcdServerProcessConfig{execPath: cfg.execPath, args: args, tlsArgs: cfg.tlsArgs(), dataDirPath: dataDirPath, keepDataDir: cfg.keepDataDir, name: name, purl: purl, acurl: curl, murl: murl, initialToken: cfg.initialToken}
	}
...
func (cfg *etcdProcessClusterConfig) tlsArgs() (args []string) {
	...
	if cfg.clientTLS != clientNonTLS {
		if cfg.isClientAutoTLS {
			...
			tlsClientArgs := []string{"--cert-file", certPath, "--key-file", privateKeyPath, "--ca-file", caPath}
	...
	if cfg.isPeerTLS {
		if cfg.isPeerAutoTLS {
			...
			tlsPeerArgs := []string{"--peer-cert-file", certPath, "--peer-key-file", privateKeyPath, "--peer-ca-file", caPath}
