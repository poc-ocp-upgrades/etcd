...
func getTest(cx ctlCtx) {
	...
	tests := []struct {
		args	[]string
		wkv	[]kv
	}{{[]string{"key1"}, []kv{{"key1", "val1"}}}, {[]string{"", "--prefix"}, kvs}, {[]string{"", "--from-key"}, kvs}, {[]string{"key", "--prefix"}, kvs}, {[]string{"key", "--prefix", "--limit=2"}, kvs[:2]}, {[]string{"key", "--prefix", "--order=ASCEND", "--sort-by=MODIFY"}, kvs}, {[]string{"key", "--prefix", "--order=ASCEND", "--sort-by=VERSION"}, kvs}, {[]string{"key", "--prefix", "--sort-by=CREATE"}, kvs}, {[]string{"key", "--prefix", "--order=DESCEND", "--sort-by=CREATE"}, revkvs}, {[]string{"key", "--prefix", "--order=DESCEND", "--sort-by=KEY"}, revkvs}}
...
func getFormatTest(cx ctlCtx) {
	...
	tests := []struct {
		...
		valueOnly	bool
		wstr		string
	}{{"simple", false, "abc"}, {"simple", true, "123"}, {"json", false, `"kvs":[{"key":"YWJj"`}, {"protobuf", false, "\x17\b\x93\xe7\xf6\x93\xd4ņ\xe14\x10\xed"}}
	for i, tt := range tests {
...
func getRevTest(cx ctlCtx) {
	...
	tests := []struct {
		args	[]string
		wkv	[]kv
	}{{[]string{"key", "--rev", "2"}, kvs[:1]}, {[]string{"key", "--rev", "3"}, kvs[1:2]}, {[]string{"key", "--rev", "4"}, kvs[2:]}}
	for i, tt := range tests {
		if err := ctlV3Get(cx, tt.args, tt.wkv...); err != nil {
			cx.t.Errorf("getTest #%d: ctlV3Get error (%v)", i, err)
		}
...
func delTest(cx ctlCtx) {
	...
	tests := []struct {
		...
		args		[]string
		deletedNum	int
	}{{[]kv{{"foo1", "bar"}, {"foo2", "bar"}, {"foo3", "bar"}}, []string{"", "--prefix"}, 3}, {[]kv{{"foo1", "bar"}, {"foo2", "bar"}, {"foo3", "bar"}}, []string{"", "--from-key"}, 3}, {[]kv{{"this", "value"}}, []string{"that"}, 0}, {[]kv{{"sample", "value"}}, []string{"sample"}, 1}, {[]kv{{"key1", "val1"}, {"key2", "val2"}, {"key3", "val3"}}, []string{"key", "--prefix"}, 3}, {[]kv{{"zoo1", "bar"}, {"zoo2", "bar2"}, {"zoo3", "bar3"}}, []string{"zoo1", "--from-key"}, 3}}
	for i, tt := range tests {
		for j := range tt.puts {
