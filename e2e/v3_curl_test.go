...
func testCurlPutGetGRPCGateway(t *testing.T, cfg *etcdProcessClusterConfig, pathPrefix string) {
	...
	putData, err := json.Marshal(&pb.PutRequest{Key: key, Value: value})
	if err != nil {
		t.Fatal(err)
	}
	rangeData, err := json.Marshal(&pb.RangeRequest{Key: key})
	if err != nil {
		t.Fatal(err)
	}
...
func testV3CurlTxn(t *testing.T, pathPrefix string) {
	...
	txn := &pb.TxnRequest{Compare: []*pb.Compare{{Key: []byte("foo"), Result: pb.Compare_EQUAL, Target: pb.Compare_CREATE, TargetUnion: &pb.Compare_CreateRevision{0}}}, Success: []*pb.RequestOp{{Request: &pb.RequestOp_RequestPut{RequestPut: &pb.PutRequest{Key: []byte("foo"), Value: []byte("bar")}}}}}
	m := &runtime.JSONPb{}
...
func testV3CurlCampaign(t *testing.T, pathPrefix string) {
	...
	cdata, err := json.Marshal(&epb.CampaignRequest{Name: []byte("/election-prefix"), Value: []byte("v1")})
	if err != nil {
		t.Fatal(err)
	}
	cargs := cURLPrefixArgs(epc, "POST", cURLReq{endpoint: path.Join(pathPrefix, "/election/campaign"), value: string(cdata)})
	...
	pdata, err := json.Marshal(&epb.ProclaimRequest{Leader: &epb.LeaderKey{Name: ndata, Key: kdata, Rev: rev, Lease: lease}, Value: []byte("v2")})
	if err != nil {
		t.Fatal(err)
	}
	if err = cURLPost(epc, cURLReq{endpoint: path.Join(pathPrefix, "/election/proclaim"), value: string(pdata), expected: `"revision":`}); err != nil {
...
func testV3CurlProclaimMissiongLeaderKey(cx ctlCtx) {
	...
	if err = cURLPost(cx.epc, cURLReq{endpoint: path.Join("/v3beta", "/election/proclaim"), value: string(pdata), expected: `{"error":"\"leader\" field must be provided","code":2}`}); err != nil {
...
func testV3CurlCipherSuites(t *testing.T, valid bool) {
	...
	cc := configClientTLS
	cc.clusterSize = 1
	cc.cipherSuites = []string{"TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256", "TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384", "TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256", "TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384", "TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305", "TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305"}
