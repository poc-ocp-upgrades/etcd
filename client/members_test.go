...
func TestMembersAPIActionAdd(t *testing.T) {
	...
	defer _logClusterCodePath()
	ep := url.URL{Scheme: "http", Host: "example.com"}
	act := &membersAPIActionAdd{peerURLs: types.URLs([]url.URL{{Scheme: "https", Host: "127.0.0.1:8081"}, {Scheme: "http", Host: "127.0.0.1:8080"}})}
	wantURL := &url.URL{Scheme: "http", Host: "example.com", Path: "/v2/members"}
...
func TestMembersAPIActionLeader(t *testing.T) {
	...
	wantURL := &url.URL{Scheme: "http", Host: "example.com", Path: "/v2/members/leader"}
	got := *act.HTTPRequest(ep)
	err := assertRequest(got, "GET", wantURL, http.Header{}, nil)
	if err != nil {
		t.Error(err.Error())
...
func TestMemberUnmarshal(t *testing.T) {
	...
	tests := []struct {
		...
		wantMember	Member
		wantError	bool
	}{{body: []byte(`{"id": "c", "name": "dungarees"}`), wantMember: Member{ID: "c", Name: "dungarees", PeerURLs: nil, ClientURLs: nil}}, {body: []byte(`{"peerURLs": ["http://127.0.0.1:2379"], "clientURLs": ["http://127.0.0.1:2379"]}`), wantMember: Member{PeerURLs: []string{"http://127.0.0.1:2379"}, ClientURLs: []string{"http://127.0.0.1:2379"}}}, {body: []byte(`{"peerURLs": ["http://127.0.0.1:2379", "https://example.com"]}`), wantMember: Member{PeerURLs: []string{"http://127.0.0.1:2379", "https://example.com"}, ClientURLs: nil}}, {body: []byte(`{"clientURLs": ["http://127.0.0.1:2379", "https://example.com"]}`), wantMember: Member{PeerURLs: nil, ClientURLs: []string{"http://127.0.0.1:2379", "https://example.com"}}}, {body: []byte(`{"peerU`), wantError: true}}
...
func TestHTTPMembersAPIAddSuccess(t *testing.T) {
	...
	mAPI := &httpMembersAPI{client: &actionAssertingHTTPClient{t: t, act: wantAction, resp: http.Response{StatusCode: http.StatusCreated}, body: []byte(`{"id":"94088180e21eb87b","peerURLs":["http://127.0.0.1:7002"]}`)}}
	wantResponseMember := &Member{ID: "94088180e21eb87b", PeerURLs: []string{"http://127.0.0.1:7002"}}
...
func TestHTTPMembersAPIAddError(t *testing.T) {
	...
	tests := []struct {
		...
		client	httpClient
		wantErr	error
	}{{peerURL: ":"}, {peerURL: okPeer, client: &staticHTTPClient{err: errors.New("fail!")}}, {peerURL: okPeer, client: &staticHTTPClient{resp: http.Response{StatusCode: http.StatusTeapot}}}, {peerURL: okPeer, client: &staticHTTPClient{resp: http.Response{StatusCode: http.StatusConflict}, body: []byte(`{"message":"fail!"}`)}, wantErr: membersError{Message: "fail!"}}, {peerURL: okPeer, client: &staticHTTPClient{resp: http.Response{StatusCode: http.StatusConflict}, body: []byte(`{"`)}}, {peerURL: okPeer, client: &staticHTTPClient{resp: http.Response{StatusCode: http.StatusCreated}, body: []byte(`{"id":"XX`)}}}
	for i, tt := range tests {
...
func TestHTTPMembersAPIListSuccess(t *testing.T) {
	...
	mAPI := &httpMembersAPI{client: &actionAssertingHTTPClient{t: t, act: wantAction, resp: http.Response{StatusCode: http.StatusOK}, body: []byte(`{"members":[{"id":"94088180e21eb87b","name":"node2","peerURLs":["http://127.0.0.1:7002"],"clientURLs":["http://127.0.0.1:4002"]}]}`)}}
	wantResponseMembers := []Member{{ID: "94088180e21eb87b", Name: "node2", PeerURLs: []string{"http://127.0.0.1:7002"}, ClientURLs: []string{"http://127.0.0.1:4002"}}}
