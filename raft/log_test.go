...
func TestIsUpToDate(t *testing.T) {
	...
	tests := []struct {
		...
		term		uint64
		wUpToDate	bool
	}{{raftLog.lastIndex() - 1, 4, true}, {raftLog.lastIndex(), 4, true}, {raftLog.lastIndex() + 1, 4, true}, {raftLog.lastIndex() - 1, 2, false}, {raftLog.lastIndex(), 2, false}, {raftLog.lastIndex() + 1, 2, false}, {raftLog.lastIndex() - 1, 3, false}, {raftLog.lastIndex(), 3, true}, {raftLog.lastIndex() + 1, 3, true}}
	for i, tt := range tests {
...
func TestAppend(t *testing.T) {
	...
	tests := []struct {
		...
		wents		[]pb.Entry
		wunstable	uint64
	}{{[]pb.Entry{}, 2, []pb.Entry{{Index: 1, Term: 1}, {Index: 2, Term: 2}}, 3}, {[]pb.Entry{{Index: 3, Term: 2}}, 3, []pb.Entry{{Index: 1, Term: 1}, {Index: 2, Term: 2}, {Index: 3, Term: 2}}, 3}, {[]pb.Entry{{Index: 1, Term: 2}}, 1, []pb.Entry{{Index: 1, Term: 2}}, 1}, {[]pb.Entry{{Index: 2, Term: 3}, {Index: 3, Term: 3}}, 3, []pb.Entry{{Index: 1, Term: 1}, {Index: 2, Term: 3}, {Index: 3, Term: 3}}, 2}}
	for i, tt := range tests {
...
func TestLogMaybeAppend(t *testing.T) {
	...
	tests := []struct {
		...
		wcommit		uint64
		wpanic		bool
	}{{lastterm - 1, lastindex, lastindex, []pb.Entry{{Index: lastindex + 1, Term: 4}}, 0, false, commit, false}, {lastterm, lastindex + 1, lastindex, []pb.Entry{{Index: lastindex + 2, Term: 4}}, 0, false, commit, false}, {lastterm, lastindex, lastindex, nil, lastindex, true, lastindex, false}, {lastterm, lastindex, lastindex + 1, nil, lastindex, true, lastindex, false}, {lastterm, lastindex, lastindex - 1, nil, lastindex, true, lastindex - 1, false}, {lastterm, lastindex, 0, nil, lastindex, true, commit, false}, {0, 0, lastindex, nil, 0, true, commit, false}, {lastterm, lastindex, lastindex, []pb.Entry{{Index: lastindex + 1, Term: 4}}, lastindex + 1, true, lastindex, false}, {lastterm, lastindex, lastindex + 1, []pb.Entry{{Index: lastindex + 1, Term: 4}}, lastindex + 1, true, lastindex + 1, false}, {lastterm, lastindex, lastindex + 2, []pb.Entry{{Index: lastindex + 1, Term: 4}}, lastindex + 1, true, lastindex + 1, false}, {lastterm, lastindex, lastindex + 2, []pb.Entry{{Index: lastindex + 1, Term: 4}, {Index: lastindex + 2, Term: 4}}, lastindex + 2, true, lastindex + 2, false}, {lastterm - 1, lastindex - 1, lastindex, []pb.Entry{{Index: lastindex, Term: 4}}, lastindex, true, lastindex, false}, {lastterm - 2, lastindex - 2, lastindex, []pb.Entry{{Index: lastindex - 1, Term: 4}}, lastindex - 1, true, lastindex - 1, false}, {lastterm - 3, lastindex - 3, lastindex, []pb.Entry{{Index: lastindex - 2, Term: 4}}, lastindex - 2, true, lastindex - 2, true}, {lastterm - 2, lastindex - 2, lastindex, []pb.Entry{{Index: lastindex - 1, Term: 4}, {Index: lastindex, Term: 4}}, lastindex, true, lastindex, false}}
	for i, tt := range tests {
		raftLog := newLog(NewMemoryStorage(), raftLogger)
		raftLog.append(previousEnts...)
...
func TestCompaction(t *testing.T) {
	...
	tests := []struct {
		...
		wleft		[]int
		wallow		bool
	}{{1000, []uint64{1001}, []int{-1}, false}, {1000, []uint64{300, 500, 800, 900}, []int{700, 500, 200, 100}, true}, {1000, []uint64{300, 299}, []int{700, -1}, false}}
	for i, tt := range tests {
		func() {
			defer func() {
				if r := recover(); r != nil {
...
func TestIsOutOfBounds(t *testing.T) {
	...
	tests := []struct {
		...
		wpanic		bool
		wErrCompacted	bool
	}{{first - 2, first + 1, false, true}, {first - 1, first + 1, false, true}, {first, first, false, false}, {first + num/2, first + num/2, false, false}, {first + num - 1, first + num - 1, false, false}, {first + num, first + num, false, false}, {first + num, first + num + 1, true, false}, {first + num + 1, first + num + 1, true, false}}
	for i, tt := range tests {
		func() {
			defer func() {
				if r := recover(); r != nil {
...
func TestTermWithUnstableSnapshot(t *testing.T) {
	...
	tests := []struct {
		index	uint64
		w	uint64
	}{{storagesnapi, 0}, {storagesnapi + 1, 0}, {unstablesnapi - 1, 0}, {unstablesnapi, 1}}
	for i, tt := range tests {
		term := mustTerm(l.term(tt.index))
		if term != tt.w {
