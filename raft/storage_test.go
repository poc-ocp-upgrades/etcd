...
func TestStorageTerm(t *testing.T) {
	...
	tests := []struct {
		...
		wterm	uint64
		wpanic	bool
	}{{2, ErrCompacted, 0, false}, {3, nil, 3, false}, {4, nil, 4, false}, {5, nil, 5, false}, {6, ErrUnavailable, 0, false}}
...
func TestStorageCompact(t *testing.T) {
	...
	tests := []struct {
		...
		wterm	uint64
		wlen	int
	}{{2, ErrCompacted, 3, 3, 3}, {3, ErrCompacted, 3, 3, 3}, {4, nil, 4, 4, 2}, {5, nil, 5, 5, 1}}
	for i, tt := range tests {
		s := &MemoryStorage{ents: ents}
		err := s.Compact(tt.i)
		if err != tt.werr {
...
func TestStorageCreateSnapshot(t *testing.T) {
	...
	tests := []struct {
		...
		werr	error
		wsnap	pb.Snapshot
	}{{4, nil, pb.Snapshot{Data: data, Metadata: pb.SnapshotMetadata{Index: 4, Term: 4, ConfState: *cs}}}, {5, nil, pb.Snapshot{Data: data, Metadata: pb.SnapshotMetadata{Index: 5, Term: 5, ConfState: *cs}}}}
	for i, tt := range tests {
		s := &MemoryStorage{ents: ents}
...
func TestStorageApplySnapshot(t *testing.T) {
	...
	cs := &pb.ConfState{Nodes: []uint64{1, 2, 3}}
	data := []byte("data")
	tests := []pb.Snapshot{{Data: data, Metadata: pb.SnapshotMetadata{Index: 4, Term: 4, ConfState: *cs}}, {Data: data, Metadata: pb.SnapshotMetadata{Index: 3, Term: 3, ConfState: *cs}}}
	s := NewMemoryStorage()
	i := 0
	tt := tests[i]
