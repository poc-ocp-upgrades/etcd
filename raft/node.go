...
func newNode() node {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return node{propc: make(chan msgWithResult), recvc: make(chan pb.Message), confc: make(chan pb.ConfChange), confstatec: make(chan pb.ConfState), readyc: make(chan Ready), advancec: make(chan struct{}), tickc: make(chan struct{}, 128), done: make(chan struct{}), stop: make(chan struct{}), status: make(chan chan Status)}
}
func (n *node) Stop() {
	_logClusterCodePath()
	defer _logClusterCodePath()
