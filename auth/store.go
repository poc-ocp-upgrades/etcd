...
func (as *authStore) Authenticate(ctx context.Context, username, password string) (*pb.AuthenticateResponse, error) {
	...
	if as.lg != nil {
		as.lg.Debug("authenticated a user", zap.String("user-name", username), zap.String("token", token))
	} else {
...
func (as *authStore) UserAdd(r *pb.AuthUserAddRequest) (*pb.AuthUserAddResponse, error) {
	...
	if err != nil {
		if as.lg != nil {
			as.lg.Warn("failed to bcrypt hash password", zap.String("user-name", r.Name), zap.Error(err))
		} else {
...
func (as *authStore) UserDelete(r *pb.AuthUserDeleteRequest) (*pb.AuthUserDeleteResponse, error) {
	...
	if as.lg != nil {
		as.lg.Info("deleted a user", zap.String("user-name", r.Name), zap.Strings("user-roles", user.Roles))
	} else {
...
func (as *authStore) UserChangePassword(r *pb.AuthUserChangePasswordRequest) (*pb.AuthUserChangePasswordResponse, error) {
	...
	as.tokenProvider.invalidateUser(r.Name)
	if as.lg != nil {
		as.lg.Info("changed a password of a user", zap.String("user-name", r.Name), zap.Strings("user-roles", user.Roles))
	} else {
...
func (as *authStore) UserGrantRole(r *pb.AuthUserGrantRoleRequest) (*pb.AuthUserGrantRoleResponse, error) {
	...
	if idx < len(user.Roles) && user.Roles[idx] == r.Role {
		if as.lg != nil {
			as.lg.Warn("ignored grant role request to a user", zap.String("user-name", r.User), zap.Strings("user-roles", user.Roles), zap.String("duplicate-role-name", r.Role))
		} else {
	...
	as.commitRevision(tx)
	if as.lg != nil {
		as.lg.Info("granted a role to a user", zap.String("user-name", r.User), zap.Strings("user-roles", user.Roles), zap.String("added-role-name", r.Role))
	} else {
...
func (as *authStore) UserRevokeRole(r *pb.AuthUserRevokeRoleRequest) (*pb.AuthUserRevokeRoleResponse, error) {
	...
	if as.enabled && r.Name == rootUser && r.Role == rootRole {
		if as.lg != nil {
			as.lg.Warn("'root' user cannot revoke 'root' role", zap.String("user-name", r.Name), zap.String("role-name", r.Role))
		} else {
	...
	as.commitRevision(tx)
	if as.lg != nil {
		as.lg.Info("revoked a role from a user", zap.String("user-name", r.Name), zap.Strings("old-user-roles", user.Roles), zap.Strings("new-user-roles", updatedUser.Roles), zap.String("revoked-role-name", r.Role))
	} else {
...
func (as *authStore) RoleRevokePermission(r *pb.AuthRoleRevokePermissionRequest) (*pb.AuthRoleRevokePermissionResponse, error) {
	...
	as.commitRevision(tx)
	if as.lg != nil {
		as.lg.Info("revoked a permission on range", zap.String("role-name", r.Role), zap.String("key", string(r.Key)), zap.String("range-end", string(r.RangeEnd)))
	} else {
...
func (as *authStore) RoleGrantPermission(r *pb.AuthRoleGrantPermissionRequest) (*pb.AuthRoleGrantPermissionResponse, error) {
	...
	if idx < len(role.KeyPermission) && bytes.Equal(role.KeyPermission[idx].Key, r.Perm.Key) && bytes.Equal(role.KeyPermission[idx].RangeEnd, r.Perm.RangeEnd) {
		...
		newPerm := &authpb.Permission{Key: r.Perm.Key, RangeEnd: r.Perm.RangeEnd, PermType: r.Perm.PermType}
	...
	as.commitRevision(tx)
	if as.lg != nil {
		as.lg.Info("granted/updated a permission to a user", zap.String("user-name", r.Name), zap.String("permission-name", authpb.Permission_Type_name[int32(r.Perm.PermType)]))
	} else {
...
func getUser(lg *zap.Logger, tx backend.BatchTx, username string) *authpb.User {
	...
	if err != nil {
		if lg != nil {
			lg.Panic("failed to unmarshal 'authpb.User'", zap.String("user-name", username), zap.Error(err))
		} else {
...
func putRole(lg *zap.Logger, tx backend.BatchTx, role *authpb.Role) {
	...
	if err != nil {
		if lg != nil {
			lg.Panic("failed to marshal 'authpb.Role'", zap.String("role-name", string(role.Name)), zap.Error(err))
		} else {
...
func NewAuthStore(lg *zap.Logger, be backend.Backend, tp TokenProvider, bcryptCost int) *authStore {
	...
	if bcryptCost < bcrypt.MinCost || bcryptCost > bcrypt.MaxCost {
		if lg != nil {
			lg.Warn("use default bcrypt cost instead of the invalid given cost", zap.Int("min-cost", bcrypt.MinCost), zap.Int("max-cost", bcrypt.MaxCost), zap.Int("default-cost", bcrypt.DefaultCost), zap.Int("given-cost", bcryptCost))
		} else {
			plog.Warningf("Use default bcrypt-cost %d instead of the invalid value %d", bcrypt.DefaultCost, bcryptCost)
		}
...
func (as *authStore) AuthInfoFromTLS(ctx context.Context) (ai *AuthInfo) {
	...
	for _, chains := range tlsInfo.State.VerifiedChains {
		...
		ai = &AuthInfo{Username: chains[0].Subject.CommonName, Revision: as.Revision()}
		...
		if gw := md["grpcgateway-accept"]; len(gw) > 0 {
			if as.lg != nil {
				as.lg.Warn("ignoring common name in gRPC-gateway proxy request", zap.String("common-name", ai.Username), zap.String("user-name", ai.Username), zap.Uint64("revision", ai.Revision))
			} else {
		...
		if as.lg != nil {
			as.lg.Debug("found command name", zap.String("common-name", ai.Username), zap.String("user-name", ai.Username), zap.Uint64("revision", ai.Revision))
		} else {
...
func decomposeOpts(lg *zap.Logger, optstr string) (string, map[string]string, error) {
	...
	for i := 1; i < len(opts); i++ {
		...
		if _, ok := typeSpecificOpts[pair[0]]; ok {
			if lg != nil {
				lg.Warn("invalid token option", zap.String("option", optstr), zap.String("duplicate-parameter", pair[0]))
			} else {
...
func (as *authStore) WithRoot(ctx context.Context) context.Context {
	...
	if ts, ok := as.tokenProvider.(*tokenSimple); ok && ts != nil {
		...
		if err != nil {
			if as.lg != nil {
				as.lg.Warn("failed to generate prefix of internally used token", zap.Error(err))
			} else {
	...
	if err != nil {
		if as.lg != nil {
			as.lg.Warn("failed to assign token for lease revoking", zap.Error(err))
		} else {
...
func (as *authStore) HasRole(user, role string) bool {
	...
	if u == nil {
		if as.lg != nil {
			as.lg.Warn("'has-role' requested for non-existing user", zap.String("user-name", user), zap.String("role-name", role))
		} else {
