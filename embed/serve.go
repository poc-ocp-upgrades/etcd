...
func (sctx *serveCtx) serve(s *etcdserver.EtcdServer, tlsinfo *transport.TLSInfo, handler http.Handler, errHandler func(error), gopts ...grpc.ServerOption) (err error) {
	...
	if sctx.insecure {
		...
		srvhttp := &http.Server{Handler: wrapMux(httpmux), ErrorLog: logger}
	...
	if sctx.secure {
		...
		httpmux := sctx.createMux(gwmux, handler)
		srv := &http.Server{Handler: wrapMux(httpmux), TLSConfig: tlscfg, ErrorLog: logger}
		go func() {
...
func (sctx *serveCtx) createMux(gwmux *gw.ServeMux, handler http.Handler) *http.ServeMux {
	...
	httpmux.Handle("/v3beta/", wsproxy.WebsocketProxy(gwmux, wsproxy.WithRequestMutator(func(incoming *http.Request, outgoing *http.Request) *http.Request {
...
type v3alphaMutator struct{ mux *http.ServeMux }
