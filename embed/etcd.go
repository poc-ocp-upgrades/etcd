...
func StartEtcd(inCfg *Config) (e *Etcd, err error) {
	...
	if e.cfg.logger != nil {
		e.cfg.logger.Info("configuring peer listeners", zap.Strings("listen-peer-urls", e.cfg.getLPURLs()))
	}
	if e.Peers, err = configurePeerListeners(cfg); err != nil {
		return e, err
	}
	if e.cfg.logger != nil {
		e.cfg.logger.Info("configuring client listeners", zap.Strings("listen-client-urls", e.cfg.getLCURLs()))
	}
	...
	if e.cfg.logger != nil {
		e.cfg.logger.Info("now serving peer/client/metrics", zap.String("local-member-id", e.Server.ID().String()), zap.Strings("initial-advertise-peer-urls", e.cfg.getAPURLs()), zap.Strings("listen-peer-urls", e.cfg.getLPURLs()), zap.Strings("advertise-client-urls", e.cfg.getACURLs()), zap.Strings("listen-client-urls", e.cfg.getLCURLs()), zap.Strings("listen-metrics-urls", e.cfg.getMetricsURLs()))
	}
	serving = true
	return e, nil
}
...
func (e *Etcd) Close() {
	_logClusterCodePath()
	defer _logClusterCodePath()
	fields := []zap.Field{zap.String("name", e.cfg.Name), zap.String("data-dir", e.cfg.Dir), zap.Strings("advertise-peer-urls", e.cfg.getAPURLs()), zap.Strings("advertise-client-urls", e.cfg.getACURLs())}
	lg := e.GetLogger()
	if lg != nil {
	...
	for _, u := range cfg.LCUrls {
		...
		if fdLimit, fderr := runtimeutil.FDLimit(); fderr == nil {
			if fdLimit <= reservedInternalFDNum {
				if cfg.logger != nil {
					cfg.logger.Fatal("file descriptor limit of etcd process is too low; please set higher", zap.Uint64("limit", fdLimit), zap.Int("recommended-limit", reservedInternalFDNum))
				} else {
		...
		defer func() {
			...
			sctx.l.Close()
			if cfg.logger != nil {
				cfg.logger.Warn("closing peer listener", zap.String("address", u.Host), zap.Error(err))
			} else {
...
func (e *Etcd) serveClients() (err error) {
	...
	if !e.cfg.ClientTLSInfo.Empty() {
		if e.cfg.logger != nil {
			e.cfg.logger.Info("starting with client TLS", zap.String("tls-info", fmt.Sprintf("%+v", e.cfg.ClientTLSInfo)), zap.Strings("cipher-suites", e.cfg.CipherSuites))
		} else {
	...
	if len(e.cfg.ListenMetricsUrls) > 0 {
		...
		for _, murl := range e.cfg.ListenMetricsUrls {
			...
			go func(u url.URL, ln net.Listener) {
				if e.cfg.logger != nil {
					e.cfg.logger.Info("serving metrics", zap.String("address", u.String()))
				} else {
