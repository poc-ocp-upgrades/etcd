...
func TestV3PutOverwrite(t *testing.T) {
	...
	if respput2.Header.Revision <= respput.Header.Revision {
		t.Fatalf("expected newer revision on overwrite, got %v <= %v", respput2.Header.Revision, respput.Header.Revision)
	}
	...
	if kv.ModRevision <= kv.CreateRevision {
		t.Errorf("expected modRev > createRev, got %d <= %d", kv.ModRevision, kv.CreateRevision)
	}
...
func TestV3TxnTooManyOps(t *testing.T) {
	...
	addCompareOps := func(txn *pb.TxnRequest) {
		txn.Compare = append(txn.Compare, &pb.Compare{Result: pb.Compare_GREATER, Target: pb.Compare_CREATE, Key: keyf()})
	}
	addSuccessOps := func(txn *pb.TxnRequest) {
		txn.Success = append(txn.Success, &pb.RequestOp{Request: &pb.RequestOp_RequestPut{RequestPut: &pb.PutRequest{Key: keyf(), Value: []byte("bar")}}})
	}
	addFailureOps := func(txn *pb.TxnRequest) {
		txn.Failure = append(txn.Failure, &pb.RequestOp{Request: &pb.RequestOp_RequestPut{RequestPut: &pb.PutRequest{Key: keyf(), Value: []byte("bar")}}})
	}
	addTxnOps := func(txn *pb.TxnRequest) {
		newTxn := &pb.TxnRequest{}
		addSuccessOps(newTxn)
		txn.Success = append(txn.Success, &pb.RequestOp{Request: &pb.RequestOp_RequestTxn{RequestTxn: newTxn}})
	}
	tests := []func(txn *pb.TxnRequest){addCompareOps, addSuccessOps, addFailureOps, addTxnOps}
...
func TestV3TxnDuplicateKeys(t *testing.T) {
	...
	delKeyReq := &pb.RequestOp{Request: &pb.RequestOp_RequestDeleteRange{RequestDeleteRange: &pb.DeleteRangeRequest{Key: []byte("abc")}}}
	delInRangeReq := &pb.RequestOp{Request: &pb.RequestOp_RequestDeleteRange{RequestDeleteRange: &pb.DeleteRangeRequest{Key: []byte("a"), RangeEnd: []byte("b")}}}
	delOutOfRangeReq := &pb.RequestOp{Request: &pb.RequestOp_RequestDeleteRange{RequestDeleteRange: &pb.DeleteRangeRequest{Key: []byte("abb"), RangeEnd: []byte("abc")}}}
	txnDelReq := &pb.RequestOp{Request: &pb.RequestOp_RequestTxn{RequestTxn: &pb.TxnRequest{Success: []*pb.RequestOp{delInRangeReq}}}}
	txnDelReqTwoSide := &pb.RequestOp{Request: &pb.RequestOp_RequestTxn{RequestTxn: &pb.TxnRequest{Success: []*pb.RequestOp{delInRangeReq}, Failure: []*pb.RequestOp{delInRangeReq}}}}
	txnPutReq := &pb.RequestOp{Request: &pb.RequestOp_RequestTxn{RequestTxn: &pb.TxnRequest{Success: []*pb.RequestOp{putreq}}}}
	txnPutReqTwoSide := &pb.RequestOp{Request: &pb.RequestOp_RequestTxn{RequestTxn: &pb.TxnRequest{Success: []*pb.RequestOp{putreq}, Failure: []*pb.RequestOp{putreq}}}}
	kvc := toGRPC(clus.RandClient()).KV
	tests := []struct {
		txnSuccess	[]*pb.RequestOp
		werr		error
	}{{txnSuccess: []*pb.RequestOp{putreq, putreq}, werr: rpctypes.ErrGRPCDuplicateKey}, {txnSuccess: []*pb.RequestOp{putreq, delKeyReq}, werr: rpctypes.ErrGRPCDuplicateKey}, {txnSuccess: []*pb.RequestOp{putreq, delInRangeReq}, werr: rpctypes.ErrGRPCDuplicateKey}, {txnSuccess: []*pb.RequestOp{putreq, txnDelReq}, werr: rpctypes.ErrGRPCDuplicateKey}, {txnSuccess: []*pb.RequestOp{delInRangeReq, txnPutReq}, werr: rpctypes.ErrGRPCDuplicateKey}, {txnSuccess: []*pb.RequestOp{txnPutReqTwoSide, txnPutReqTwoSide}, werr: rpctypes.ErrGRPCDuplicateKey}, {txnSuccess: []*pb.RequestOp{delOutOfRangeReq, txnPutReqTwoSide}, werr: nil}, {txnSuccess: []*pb.RequestOp{txnDelReq, txnDelReqTwoSide}, werr: nil}, {txnSuccess: []*pb.RequestOp{delKeyReq, delInRangeReq, delKeyReq, delInRangeReq}, werr: nil}, {txnSuccess: []*pb.RequestOp{putreq, delOutOfRangeReq}, werr: nil}}
	for i, tt := range tests {
...
func TestV3TxnCmpHeaderRev(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		txnget := &pb.RequestOp{Request: &pb.RequestOp_RequestRange{RequestRange: &pb.RangeRequest{Key: []byte("k")}}}
		txn := &pb.TxnRequest{Success: []*pb.RequestOp{txnget}}
		cmp := &pb.Compare{Result: pb.Compare_EQUAL, Target: pb.Compare_VERSION, Key: []byte("k"), TargetUnion: &pb.Compare_Version{Version: int64(i)}}
...
func TestV3TxnRangeCompare(t *testing.T) {
	...
	tests := []struct {
		cmp		pb.Compare
		wSuccess	bool
	}{{pb.Compare{Key: []byte("/a/"), RangeEnd: []byte{0}, Target: pb.Compare_CREATE, Result: pb.Compare_LESS, TargetUnion: &pb.Compare_CreateRevision{6}}, true}, {pb.Compare{Key: []byte("/a/"), RangeEnd: []byte{0}, Target: pb.Compare_CREATE, Result: pb.Compare_LESS, TargetUnion: &pb.Compare_CreateRevision{5}}, false}, {pb.Compare{Key: []byte("/a/"), RangeEnd: []byte("/a0"), Target: pb.Compare_CREATE, Result: pb.Compare_LESS, TargetUnion: &pb.Compare_CreateRevision{5}}, true}, {pb.Compare{Key: []byte("/a/"), RangeEnd: []byte("/a0"), Target: pb.Compare_CREATE, Result: pb.Compare_LESS, TargetUnion: &pb.Compare_CreateRevision{4}}, false}, {pb.Compare{Key: []byte("/b/"), RangeEnd: []byte("/b0"), Target: pb.Compare_VALUE, Result: pb.Compare_EQUAL, TargetUnion: &pb.Compare_Value{[]byte("x")}}, false}, {pb.Compare{Key: []byte("/a/"), RangeEnd: []byte("/a0"), Target: pb.Compare_LEASE, Result: pb.Compare_GREATER, TargetUnion: &pb.Compare_Lease{0}}, false}, {pb.Compare{Key: []byte("/a/"), RangeEnd: []byte("/a0"), Target: pb.Compare_LEASE, Result: pb.Compare_EQUAL, TargetUnion: &pb.Compare_Lease{0}}, true}}
...
func TestV3TxnNestedPath(t *testing.T) {
	...
	defer clus.Terminate(t)
	kvc := toGRPC(clus.RandClient()).KV
	cmpTrue := &pb.Compare{Result: pb.Compare_EQUAL, Target: pb.Compare_VERSION, Key: []byte("k"), TargetUnion: &pb.Compare_Version{Version: int64(0)}}
	cmpFalse := &pb.Compare{Result: pb.Compare_EQUAL, Target: pb.Compare_VERSION, Key: []byte("k"), TargetUnion: &pb.Compare_Version{Version: int64(1)}}
...
func TestV3PutIgnoreValue(t *testing.T) {
	...
	tests := []struct {
		...
		return err
	}, rpctypes.ErrGRPCKeyNotFound, 0}, {func() error {
		preq := putReq
		preq.Value = nil
		preq.IgnoreValue = true
		txn := &pb.TxnRequest{}
		txn.Success = append(txn.Success, &pb.RequestOp{Request: &pb.RequestOp_RequestPut{RequestPut: &preq}})
		...
		_, err := kvc.Txn(context.TODO(), txn)
		return err
	}, nil, lresp.ID}, {func() error {
		preq := putReq
		preq.IgnoreValue = true
		_, err := kvc.Put(context.TODO(), &preq)
		return err
	}, rpctypes.ErrGRPCValueProvided, 0}, {func() error {
		preq := putReq
		preq.Value = nil
		preq.IgnoreValue = true
...
func TestV3PutIgnoreLease(t *testing.T) {
	...
	tests := []struct {
		...
		_, err := kvc.Put(context.TODO(), &preq)
		return err
	}, rpctypes.ErrGRPCKeyNotFound, 0, nil}, {func() error {
		preq := putReq
		preq.IgnoreLease = true
		txn := &pb.TxnRequest{}
		txn.Success = append(txn.Success, &pb.RequestOp{Request: &pb.RequestOp_RequestPut{RequestPut: &preq}})
		...
		_, err := kvc.Txn(context.TODO(), txn)
		return err
	}, nil, lresp.ID, val1}, {func() error {
		preq := putReq
		preq.Lease = lresp.ID
		preq.IgnoreLease = true
		_, err := kvc.Put(context.TODO(), &preq)
		return err
	}, rpctypes.ErrGRPCLeaseProvided, 0, nil}, {func() error {
...
func TestV3PutMissingLease(t *testing.T) {
	...
	tests := []func(){func() {
		...
		txn := &pb.TxnRequest{}
		txn.Failure = append(txn.Failure, &pb.RequestOp{Request: &pb.RequestOp_RequestPut{RequestPut: preq}})
		cmp := &pb.Compare{Result: pb.Compare_GREATER, Target: pb.Compare_CREATE, Key: []byte("bar")}
		txn.Compare = append(txn.Compare, cmp)
		if tresp, err := kvc.Txn(context.TODO(), txn); err == nil {
		...
		txn.Success = append(txn.Success, &pb.RequestOp{Request: &pb.RequestOp_RequestRange{RequestRange: rreq}})
		txn.Failure = append(txn.Failure, &pb.RequestOp{Request: &pb.RequestOp_RequestPut{RequestPut: preq}})
		if tresp, err := kvc.Txn(context.TODO(), txn); err != nil {
...
func TestV3DeleteRange(t *testing.T) {
	...
	tests := []struct {
		...
		wantSet	[][]byte
		deleted	int64
	}{{[]string{"foo", "foo/abc", "fop"}, "foo/", "fop", false, [][]byte{[]byte("foo"), []byte("fop")}, 1}, {[]string{"foo", "foo/abc", "fop"}, "foo/", "foo/", false, [][]byte{[]byte("foo"), []byte("foo/abc"), []byte("fop")}, 0}, {[]string{"foo", "foo/abc", "fop"}, "fo", "fop", false, [][]byte{[]byte("fop")}, 2}, {[]string{"foo", "foo/abc", "fop"}, "foo/", "fos", false, [][]byte{[]byte("foo")}, 2}, {[]string{"foo", "foo/abc", "fop"}, "foo/abc", "", false, [][]byte{[]byte("foo"), []byte("fop")}, 1}, {[]string{"foo"}, "foo", "foo", false, [][]byte{[]byte("foo")}, 0}, {[]string{"foo", "foo/abc", "fop"}, "foo/", "fop", true, [][]byte{[]byte("foo"), []byte("fop")}, 1}}
	for i, tt := range tests {
		clus := NewClusterV3(t, &ClusterConfig{Size: 3})
		kvc := toGRPC(clus.RandClient()).KV
		ks := tt.keySet
		...
		dreq := &pb.DeleteRangeRequest{Key: []byte(tt.begin), RangeEnd: []byte(tt.end), PrevKv: tt.prevKV}
		...
		if dresp.Header.Revision != rresp.Header.Revision {
			t.Errorf("expected revision %v, got %v", dresp.Header.Revision, rresp.Header.Revision)
		}
		keys := [][]byte{}
...
func TestV3StorageQuotaAPI(t *testing.T) {
	...
	puttxn := &pb.RequestOp{Request: &pb.RequestOp_RequestPut{RequestPut: &pb.PutRequest{Key: key, Value: bigbuf}}}
...
func TestV3RangeRequest(t *testing.T) {
	...
	tests := []struct {
		...
		wresps	[][]string
		wmores	[]bool
	}{{[]string{"foo", "bar"}, []pb.RangeRequest{{Key: []byte("foo")}, {Key: []byte("baz")}}, [][]string{{"foo"}, {}}, []bool{false, false}}, {[]string{"a", "b", "c", "d", "e"}, []pb.RangeRequest{{Key: []byte("a"), RangeEnd: []byte("z")}, {Key: []byte("b"), RangeEnd: []byte("d")}, {Key: []byte("f"), RangeEnd: []byte("z")}, {Key: []byte("c"), RangeEnd: []byte("c")}, {Key: []byte("d"), RangeEnd: []byte("b")}, {Key: []byte{0}, RangeEnd: []byte{0}}}, [][]string{{"a", "b", "c", "d", "e"}, {"b", "c"}, {}, {}, {}, {"a", "b", "c", "d", "e"}}, []bool{false, false, false, false, false, false}}, {[]string{"a", "b", "c", "d", "e"}, []pb.RangeRequest{{Key: []byte("a"), RangeEnd: []byte("z"), Revision: 0}, {Key: []byte("a"), RangeEnd: []byte("z"), Revision: 1}, {Key: []byte("a"), RangeEnd: []byte("z"), Revision: 2}, {Key: []byte("a"), RangeEnd: []byte("z"), Revision: 3}}, [][]string{{"a", "b", "c", "d", "e"}, {}, {"a"}, {"a", "b"}}, []bool{false, false, false, false}}, {[]string{"foo", "bar"}, []pb.RangeRequest{{Key: []byte("a"), RangeEnd: []byte("z"), Limit: 1}, {Key: []byte("a"), RangeEnd: []byte("z"), Limit: 2}}, [][]string{{"bar"}, {"bar", "foo"}}, []bool{true, false}}, {[]string{"b", "a", "c", "d", "c"}, []pb.RangeRequest{{Key: []byte("a"), RangeEnd: []byte("z"), Limit: 1, SortOrder: pb.RangeRequest_ASCEND, SortTarget: pb.RangeRequest_KEY}, {Key: []byte("a"), RangeEnd: []byte("z"), Limit: 1, SortOrder: pb.RangeRequest_DESCEND, SortTarget: pb.RangeRequest_KEY}, {Key: []byte("a"), RangeEnd: []byte("z"), Limit: 1, SortOrder: pb.RangeRequest_ASCEND, SortTarget: pb.RangeRequest_CREATE}, {Key: []byte("a"), RangeEnd: []byte("z"), Limit: 1, SortOrder: pb.RangeRequest_DESCEND, SortTarget: pb.RangeRequest_MOD}, {Key: []byte("z"), RangeEnd: []byte("z"), Limit: 1, SortOrder: pb.RangeRequest_DESCEND, SortTarget: pb.RangeRequest_CREATE}, {Key: []byte("a"), RangeEnd: []byte("z"), Limit: 10, SortOrder: pb.RangeRequest_NONE, SortTarget: pb.RangeRequest_CREATE}}, [][]string{{"a"}, {"d"}, {"b"}, {"c"}, {}, {"b", "a", "c", "d"}}, []bool{true, true, true, true, false, false}}, {[]string{"rev2", "rev3", "rev4", "rev5", "rev6"}, []pb.RangeRequest{{Key: []byte{0}, RangeEnd: []byte{0}, MinModRevision: 3}, {Key: []byte{0}, RangeEnd: []byte{0}, MaxModRevision: 3}, {Key: []byte{0}, RangeEnd: []byte{0}, MinModRevision: 3, MaxModRevision: 5}, {Key: []byte{0}, RangeEnd: []byte{0}, MaxModRevision: 10}}, [][]string{{"rev3", "rev4", "rev5", "rev6"}, {"rev2", "rev3"}, {"rev3", "rev4", "rev5"}, {"rev2", "rev3", "rev4", "rev5", "rev6"}}, []bool{false, false, false, false}}, {[]string{"rev2", "rev3", "rev2", "rev2", "rev6", "rev3"}, []pb.RangeRequest{{Key: []byte{0}, RangeEnd: []byte{0}, MinCreateRevision: 3}, {Key: []byte{0}, RangeEnd: []byte{0}, MaxCreateRevision: 3}, {Key: []byte{0}, RangeEnd: []byte{0}, MinCreateRevision: 3, MaxCreateRevision: 5}, {Key: []byte{0}, RangeEnd: []byte{0}, MaxCreateRevision: 10}}, [][]string{{"rev3", "rev6"}, {"rev2", "rev3"}, {"rev3"}, {"rev2", "rev3", "rev6"}}, []bool{false, false, false, false}}}
	for i, tt := range tests {
		clus := NewClusterV3(t, &ClusterConfig{Size: 3})
...
func testTLSReload(t *testing.T, cloneFunc func() transport.TLSInfo, replaceFunc func(), revertFunc func()) {
	...
	cl, cerr := clientv3.New(clientv3.Config{Endpoints: []string{clus.Members[0].GRPCAddr()}, DialTimeout: 5 * time.Second, TLS: tls})
	if cerr != nil {
...
func TestGRPCStreamRequireLeader(t *testing.T) {
	...
	wreq := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: []byte("foo")}}}
...
func TestV3LargeRequests(t *testing.T) {
	...
	tests := []struct {
		...
		valueSize	int
		expectError	error
	}{{1, 1024, rpctypes.ErrGRPCRequestTooLarge}, {10 * 1024 * 1024, 9 * 1024 * 1024, nil}, {10 * 1024 * 1024, 10 * 1024 * 1024, rpctypes.ErrGRPCRequestTooLarge}, {10 * 1024 * 1024, 10*1024*1024 + 5, rpctypes.ErrGRPCRequestTooLarge}}
