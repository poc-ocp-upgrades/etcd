...
func TestV3AuthWithLeaseRevokeWithRoot(t *testing.T) {
	...
	tresp, terr := api.Lease.LeaseTimeToLive(context.TODO(), &pb.LeaseTimeToLiveRequest{ID: int64(leaseID), Keys: true})
	if terr != nil {
		t.Error(terr)
	}
...
func TestV3AuthWithLeaseRevoke(t *testing.T) {
	...
	clus := NewClusterV3(t, &ClusterConfig{Size: 1})
	defer clus.Terminate(t)
	users := []user{{name: "user1", password: "user1-123", role: "role1", key: "k1", end: "k2"}}
...
func TestV3AuthWithLeaseAttach(t *testing.T) {
	...
	clus := NewClusterV3(t, &ClusterConfig{Size: 1})
	defer clus.Terminate(t)
	users := []user{{name: "user1", password: "user1-123", role: "role1", key: "k1", end: "k3"}, {name: "user2", password: "user2-123", role: "role2", key: "k2", end: "k4"}}
	authSetupUsers(t, toGRPC(clus.Client(0)).Auth, users)
	authSetupRoot(t, toGRPC(clus.Client(0)).Auth)
...
func authSetupUsers(t *testing.T, auth pb.AuthClient, users []user) {
	...
	for _, user := range users {
		...
		perm := &authpb.Permission{PermType: authpb.READWRITE, Key: []byte(user.key), RangeEnd: []byte(user.end)}
...
func TestV3AuthOldRevConcurrent(t *testing.T) {
	...
	defer clus.Terminate(t)
	authSetupRoot(t, toGRPC(clus.Client(0)).Auth)
	c, cerr := clientv3.New(clientv3.Config{Endpoints: clus.Client(0).Endpoints(), DialTimeout: 5 * time.Second, Username: "root", Password: "123"})
