...
func TestV3WatchFutureRevision(t *testing.T) {
	...
	wkey := []byte("foo")
	wrev := int64(10)
	req := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: wkey, StartRevision: wrev}}}
...
func TestV3WatchWrongRange(t *testing.T) {
	...
	tests := []struct {
		...
		canceled	bool
	}{{[]byte("a"), []byte("a"), true}, {[]byte("b"), []byte("a"), true}, {[]byte("foo"), []byte{0}, false}}
	for i, tt := range tests {
		if err := wStream.Send(&pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: tt.key, RangeEnd: tt.end, StartRevision: 1}}}); err != nil {
			t.Fatalf("#%d: wStream.Send error: %v", i, err)
		}
		cresp, err := wStream.Recv()
		if err != nil {
...
func TestV3WatchCurrentPutOverlap(t *testing.T) {
	...
	wreq := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: []byte("foo"), RangeEnd: []byte("fop")}}}
	if err := wStream.Send(wreq); err != nil {
...
func TestV3WatchEmptyKey(t *testing.T) {
	...
	req := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: []byte("foo")}}}
	...
	wevs := []*mvccpb.Event{{Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo"), CreateRevision: 2, ModRevision: 2, Version: 1}}}
...
func testV3WatchMultipleWatchers(t *testing.T, startRev int64) {
	...
	for i := 0; i < watchKeyN+1; i++ {
		...
		if i < watchKeyN {
			...
			wreq = &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: []byte("fo"), RangeEnd: []byte("fp"), StartRevision: startRev}}}
		}
		if err := wStream.Send(wreq); err != nil {
			t.Fatalf("wStream.Send error: %v", err)
		}
...
func testV3WatchMultipleEventsTxn(t *testing.T, startRev int64) {
	...
	wreq := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: []byte("foo"), RangeEnd: []byte("fop"), StartRevision: startRev}}}
	if err := wStream.Send(wreq); err != nil {
		t.Fatalf("wStream.Send error: %v", err)
	}
	...
	for i := 0; i < 3; i++ {
		ru := &pb.RequestOp{}
		ru.Request = &pb.RequestOp_RequestPut{RequestPut: &pb.PutRequest{Key: []byte(fmt.Sprintf("foo%d", i)), Value: []byte("bar")}}
	...
	wevents := []*mvccpb.Event{{Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo0"), Value: []byte("bar"), CreateRevision: 2, ModRevision: 2, Version: 1}}, {Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo1"), Value: []byte("bar"), CreateRevision: 2, ModRevision: 2, Version: 1}}, {Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo2"), Value: []byte("bar"), CreateRevision: 2, ModRevision: 2, Version: 1}}}
...
func TestV3WatchMultipleEventsPutUnsynced(t *testing.T) {
	...
	wreq := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: []byte("foo"), RangeEnd: []byte("fop"), StartRevision: 1}}}
	if err := wStream.Send(wreq); err != nil {
		t.Fatalf("wStream.Send error: %v", err)
	}
	if _, err := kvc.Put(context.TODO(), &pb.PutRequest{Key: []byte("foo0"), Value: []byte("bar")}); err != nil {
	...
	allWevents := []*mvccpb.Event{{Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo0"), Value: []byte("bar"), CreateRevision: 2, ModRevision: 2, Version: 1}}, {Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo1"), Value: []byte("bar"), CreateRevision: 3, ModRevision: 3, Version: 1}}, {Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo0"), Value: []byte("bar"), CreateRevision: 2, ModRevision: 4, Version: 2}}, {Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo1"), Value: []byte("bar"), CreateRevision: 3, ModRevision: 5, Version: 2}}}
	events := []*mvccpb.Event{}
	for len(events) < 4 {
		resp, err := wStream.Recv()
		if err != nil {
...
func testV3WatchMultipleStreams(t *testing.T, startRev int64) {
	...
	wg.Add(len(streams))
	wevents := []*mvccpb.Event{{Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo"), Value: []byte("bar"), CreateRevision: 2, ModRevision: 2, Version: 1}}}
	for i := range streams {
		go func(i int) {
			defer wg.Done()
			wStream := streams[i]
...
func TestWatchWithProgressNotify(t *testing.T) {
	...
	wreq := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: []byte("foo"), StartRevision: 1, ProgressNotify: true}}}
	if err := wStream.Send(wreq); err != nil {
		t.Fatalf("watch request failed (%v)", err)
	}
...
func TestV3WatchClose(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		go func() {
			...
			req := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: cr}}
			ws.Send(req)
			ws.Recv()
		}()
	}
...
func TestV3WatchWithFilter(t *testing.T) {
	...
	req := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: []byte("foo"), Filters: []pb.WatchCreateRequest_FilterType{pb.WatchCreateRequest_NOPUT}}}}
	if err := ws.Send(req); err != nil {
		t.Fatal(err)
	}
	if _, err := ws.Recv(); err != nil {
	...
	select {
	case resp := <-recv:
		wevs := []*mvccpb.Event{{Type: mvccpb.DELETE, Kv: &mvccpb.KeyValue{Key: []byte("foo"), ModRevision: 3}}}
		if !reflect.DeepEqual(resp.Events, wevs) {
			t.Fatalf("got %v, expected %v", resp.Events, wevs)
		}
	case <-time.After(100 * time.Millisecond):
...
func TestV3WatchWithPrevKV(t *testing.T) {
	...
	tests := []struct {
		...
		vals	[]string
	}{{key: "foo", end: "fop", vals: []string{"bar1", "bar2"}}, {key: "/abc", end: "/abd", vals: []string{"first", "second"}}}
	for i, tt := range tests {
		kvc := toGRPC(clus.RandClient()).KV
		...
		req := &pb.WatchRequest{RequestUnion: &pb.WatchRequest_CreateRequest{CreateRequest: &pb.WatchCreateRequest{Key: []byte(tt.key), RangeEnd: []byte(tt.end), PrevKv: true}}}
		if err := ws.Send(req); err != nil {
			t.Fatal(err)
		}
		if _, err := ws.Recv(); err != nil {
