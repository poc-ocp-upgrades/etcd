...
func startEtcdOrProxyV2() {
	...
	if defaultHost != "" {
		if lg != nil {
			lg.Info("detected default host for advertise", zap.String("host", defaultHost))
		} else {
	...
	if cfg.ec.Dir == "" {
		...
		if lg != nil {
			lg.Warn("'data-dir' was empty; using default", zap.String("data-dir", cfg.ec.Dir))
		} else {
	...
	if which != dirEmpty {
		if lg != nil {
			lg.Info("server has been already initialized", zap.String("data-dir", cfg.ec.Dir), zap.String("dir-type", string(which)))
		} else {
		...
		default:
			if lg != nil {
				lg.Panic("unknown directory type", zap.String("dir-type", string(which)))
			} else {
...
func startProxy(cfg *config) error {
	...
	case err == nil:
		if cfg.ec.Durl != "" {
			if lg != nil {
				lg.Warn("discovery token ignored since the proxy has already been initialized; valid cluster file found", zap.String("cluster-file", clusterfile))
			} else {
		...
		if cfg.ec.DNSCluster != "" {
			if lg != nil {
				lg.Warn("DNS SRV discovery ignored since the proxy has already been initialized; valid cluster file found", zap.String("cluster-file", clusterfile))
			} else {
		...
		if lg != nil {
			lg.Info("proxy using peer URLS from cluster file", zap.Strings("peer-urls", peerURLs), zap.String("cluster-file", clusterfile))
		} else {
	...
	uf := func() []string {
		...
		if gerr != nil {
			if lg != nil {
				lg.Warn("failed to get cluster from remote peers", zap.Strings("peer-urls", peerURLs), zap.Error(gerr))
			} else {
		...
		if err != nil {
			if lg != nil {
				lg.Warn("proxy failed to rename cluster file", zap.String("path", clusterfile), zap.Error(err))
			} else {
		...
		if !reflect.DeepEqual(gcls.PeerURLs(), peerURLs) {
			if lg != nil {
				lg.Info("proxy updated peer URLs", zap.Strings("from", peerURLs), zap.Strings("to", gcls.PeerURLs()))
			} else {
...
func identifyDataDirOrDie(lg *zap.Logger, dir string) dirType {
	...
	for _, name := range names {
		...
		default:
			if lg != nil {
				lg.Warn("found invalid file under data directory", zap.String("filename", name), zap.String("data-dir", dir))
			} else {
