...
func TestIndexGet(t *testing.T) {
	...
	tests := []struct {
		...
		wver		int64
		werr		error
	}{{0, revision{}, revision{}, 0, ErrRevisionNotFound}, {1, revision{}, revision{}, 0, ErrRevisionNotFound}, {2, revision{main: 2}, revision{main: 2}, 1, nil}, {3, revision{main: 2}, revision{main: 2}, 1, nil}, {4, revision{main: 4}, revision{main: 2}, 2, nil}, {5, revision{main: 4}, revision{main: 2}, 2, nil}, {6, revision{}, revision{}, 0, ErrRevisionNotFound}}
...
func TestIndexRange(t *testing.T) {
	...
	tests := []struct {
		...
		wkeys		[][]byte
		wrevs		[]revision
	}{{[]byte("bar"), nil, nil, nil}, {[]byte("foo"), nil, allKeys[:1], allRevs[:1]}, {[]byte("foo"), []byte("foo1"), allKeys[:1], allRevs[:1]}, {[]byte("foo"), []byte("foo2"), allKeys[:2], allRevs[:2]}, {[]byte("foo"), []byte("fop"), allKeys, allRevs}, {[]byte("foo1"), []byte("fop"), allKeys[1:], allRevs[1:]}, {[]byte("foo2"), []byte("fop"), allKeys[2:], allRevs[2:]}, {[]byte("foo3"), []byte("fop"), nil, nil}}
	for i, tt := range tests {
...
func TestIndexRangeSince(t *testing.T) {
	...
	tests := []struct {
		key, end	[]byte
		wrevs		[]revision
	}{{[]byte("bar"), nil, nil}, {[]byte("foo"), nil, []revision{{main: 1}, {main: 6}}}, {[]byte("foo"), []byte("foo1"), []revision{{main: 1}, {main: 6}}}, {[]byte("foo"), []byte("foo2"), []revision{{main: 1}, {main: 2}, {main: 5}, {main: 6}}}, {[]byte("foo"), []byte("fop"), allRevs}, {[]byte("foo1"), []byte("fop"), []revision{{main: 2}, {main: 3}, {main: 4}, {main: 5}}}, {[]byte("foo2"), []byte("fop"), []revision{{main: 3}, {main: 4}}}, {[]byte("foo3"), []byte("fop"), nil}}
	for i, tt := range tests {
...
func TestIndexCompactAndKeep(t *testing.T) {
	...
	tests := []struct {
		...
		created	revision
		ver	int64
	}{{[]byte("foo"), false, revision{main: 1}, revision{main: 1}, 1}, {[]byte("foo1"), false, revision{main: 2}, revision{main: 2}, 1}, {[]byte("foo2"), false, revision{main: 3}, revision{main: 3}, 1}, {[]byte("foo2"), false, revision{main: 4}, revision{main: 3}, 2}, {[]byte("foo"), false, revision{main: 5}, revision{main: 1}, 2}, {[]byte("foo1"), false, revision{main: 6}, revision{main: 2}, 2}, {[]byte("foo1"), true, revision{main: 7}, revision{}, 0}, {[]byte("foo2"), true, revision{main: 8}, revision{}, 0}, {[]byte("foo"), true, revision{main: 9}, revision{}, 0}, {[]byte("foo"), false, revision{10, 0}, revision{10, 0}, 1}, {[]byte("foo1"), false, revision{10, 1}, revision{10, 1}, 1}}
	ti := newTreeIndex()
