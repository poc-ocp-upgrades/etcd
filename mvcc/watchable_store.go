...
func newWatchableStore(lg *zap.Logger, b backend.Backend, le lease.Lessor, ig ConsistentIndexGetter) *watchableStore {
	_logClusterCodePath()
	defer _logClusterCodePath()
	s := &watchableStore{store: NewStore(lg, b, le, ig), victimc: make(chan struct{}, 1), unsynced: newWatcherGroup(), synced: newWatcherGroup(), stopc: make(chan struct{})}
...
func (s *watchableStore) notify(rev int64, evs []mvccpb.Event) {
	...
	for w, eb := range newWatcherBatch(&s.synced, evs) {
		if eb.revs != 1 {
			if s.store != nil && s.store.lg != nil {
				s.store.lg.Panic("unexpected multiple revisions in watch notification", zap.Int("number-of-revisions", eb.revs))
			} else {
