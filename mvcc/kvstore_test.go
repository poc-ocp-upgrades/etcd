...
func TestStorePut(t *testing.T) {
	...
	for i, tt := range tests {
		...
		wact := []testutil.Action{{"seqput", []interface{}{keyBucketName, tt.wkey, data}}}
		if tt.rr != nil {
			wact = []testutil.Action{{"seqput", []interface{}{keyBucketName, tt.wkey, data}}}
		}
		...
		wact = []testutil.Action{{"get", []interface{}{[]byte("foo"), tt.wputrev.main}}, {"put", []interface{}{[]byte("foo"), tt.wputrev}}}
...
func TestStoreRange(t *testing.T) {
	...
	tests := []struct {
		idxr	indexRangeResp
		r	rangeResp
	}{{indexRangeResp{[][]byte{[]byte("foo")}, []revision{{2, 0}}}, rangeResp{[][]byte{key}, [][]byte{kvb}}}, {indexRangeResp{[][]byte{[]byte("foo"), []byte("foo1")}, []revision{{2, 0}, {3, 0}}}, rangeResp{[][]byte{key}, [][]byte{kvb}}}}
	...
	for i, tt := range tests {
		...
		wact := []testutil.Action{{"range", []interface{}{keyBucketName, wstart, []byte(nil), int64(0)}}}
		if g := b.tx.Action(); !reflect.DeepEqual(g, wact) {
			t.Errorf("#%d: tx action = %+v, want %+v", i, g, wact)
		}
		wact = []testutil.Action{{"range", []interface{}{[]byte("foo"), []byte("goo"), wrev}}}
		if g := fi.Action(); !reflect.DeepEqual(g, wact) {
			t.Errorf("#%d: index action = %+v, want %+v", i, g, wact)
		}
		if s.currentRev != 2 {
...
func TestStoreDeleteRange(t *testing.T) {
	...
	tests := []struct {
		...
		wrrev	int64
		wdelrev	revision
	}{{revision{2, 0}, indexRangeResp{[][]byte{[]byte("foo")}, []revision{{2, 0}}}, rangeResp{[][]byte{key}, [][]byte{kvb}}, newTestKeyBytes(revision{3, 0}, true), revision{3, 0}, 2, revision{3, 0}}}
	for i, tt := range tests {
		s := newFakeStore()
		b := s.b.(*fakeBackend)
		fi := s.kvindex.(*fakeIndex)
		...
		data, err := (&mvccpb.KeyValue{Key: []byte("foo")}).Marshal()
		if err != nil {
			t.Errorf("#%d: marshal err = %v, want nil", i, err)
		}
		wact := []testutil.Action{{"seqput", []interface{}{keyBucketName, tt.wkey, data}}}
		...
		wact = []testutil.Action{{"range", []interface{}{[]byte("foo"), []byte("goo"), tt.wrrev}}, {"tombstone", []interface{}{[]byte("foo"), tt.wdelrev}}}
		if g := fi.Action(); !reflect.DeepEqual(g, wact) {
			t.Errorf("#%d: index action = %+v, want %+v", i, g, wact)
		}
		if s.currentRev != tt.wrev.main {
...
func TestStoreCompact(t *testing.T) {
	...
	wact := []testutil.Action{{"put", []interface{}{metaBucketName, scheduledCompactKeyName, newTestRevBytes(revision{3, 0})}}, {"range", []interface{}{keyBucketName, make([]byte, 17), end, int64(10000)}}, {"delete", []interface{}{keyBucketName, key2}}, {"put", []interface{}{metaBucketName, finishedCompactKeyName, newTestRevBytes(revision{3, 0})}}}
	if g := b.tx.Action(); !reflect.DeepEqual(g, wact) {
		t.Errorf("tx actions = %+v, want %+v", g, wact)
	}
	wact = []testutil.Action{{"compact", []interface{}{int64(3)}}}
	if g := fi.Action(); !reflect.DeepEqual(g, wact) {
...
func TestStoreRestore(t *testing.T) {
	...
	putkv := mvccpb.KeyValue{Key: []byte("foo"), Value: []byte("bar"), CreateRevision: 4, ModRevision: 4, Version: 1}
	putkvb, err := putkv.Marshal()
	if err != nil {
		t.Fatal(err)
	}
	delkey := newTestKeyBytes(revision{5, 0}, true)
	delkv := mvccpb.KeyValue{Key: []byte("foo")}
	...
	wact := []testutil.Action{{"range", []interface{}{metaBucketName, finishedCompactKeyName, []byte(nil), int64(0)}}, {"range", []interface{}{metaBucketName, scheduledCompactKeyName, []byte(nil), int64(0)}}, {"range", []interface{}{keyBucketName, newTestRevBytes(revision{1, 0}), newTestRevBytes(revision{math.MaxInt64, math.MaxInt64}), int64(restoreChunkKeys)}}}
	if g := b.tx.Action(); !reflect.DeepEqual(g, wact) {
		t.Errorf("tx actions = %+v, want %+v", g, wact)
	}
	gens := []generation{{created: revision{4, 0}, ver: 2, revs: []revision{{3, 0}, {5, 0}}}, {created: revision{0, 0}, ver: 0, revs: nil}}
	ki := &keyIndex{key: []byte("foo"), modified: revision{5, 0}, generations: gens}
	wact = []testutil.Action{{"keyIndex", []interface{}{ki}}, {"insert", []interface{}{ki}}}
	if g := fi.Action(); !reflect.DeepEqual(g, wact) {
		t.Errorf("index action = %+v, want %+v", g, wact)
	}
}
...
func newFakeStore() *store {
	...
	fi := &fakeIndex{Recorder: &testutil.RecorderBuffered{}, indexGetRespc: make(chan indexGetResp, 1), indexRangeRespc: make(chan indexRangeResp, 1), indexRangeEventsRespc: make(chan indexRangeEventsResp, 1), indexCompactRespc: make(chan map[revision]struct{}, 1)}
	s := &store{b: b, le: &lease.FakeLessor{}, kvindex: fi, currentRev: 0, compactMainRev: -1, fifoSched: schedule.NewFIFOScheduler(), stopc: make(chan struct{})}
...
type fakeBackend struct{ tx *fakeBatchTx }
...
type indexRangeEventsResp struct{ revs []revision }
