...
func TestKeyIndexGet(t *testing.T) {
	...
	tests := []struct {
		...
		wver	int64
		werr	error
	}{{17, revision{}, revision{}, 0, ErrRevisionNotFound}, {16, revision{}, revision{}, 0, ErrRevisionNotFound}, {15, revision{14, 1}, revision{14, 0}, 2, nil}, {14, revision{14, 1}, revision{14, 0}, 2, nil}, {13, revision{}, revision{}, 0, ErrRevisionNotFound}, {12, revision{}, revision{}, 0, ErrRevisionNotFound}, {11, revision{10, 0}, revision{8, 0}, 2, nil}, {10, revision{10, 0}, revision{8, 0}, 2, nil}, {9, revision{8, 0}, revision{8, 0}, 1, nil}, {8, revision{8, 0}, revision{8, 0}, 1, nil}, {7, revision{}, revision{}, 0, ErrRevisionNotFound}, {6, revision{}, revision{}, 0, ErrRevisionNotFound}, {5, revision{4, 0}, revision{2, 0}, 2, nil}, {4, revision{4, 0}, revision{2, 0}, 2, nil}, {3, revision{}, revision{}, 0, ErrRevisionNotFound}, {2, revision{}, revision{}, 0, ErrRevisionNotFound}, {1, revision{}, revision{}, 0, ErrRevisionNotFound}, {0, revision{}, revision{}, 0, ErrRevisionNotFound}}
...
func TestKeyIndexSince(t *testing.T) {
	...
	tests := []struct {
		rev	int64
		wrevs	[]revision
	}{{17, nil}, {16, allRevs[6:]}, {15, allRevs[6:]}, {14, allRevs[5:]}, {13, allRevs[5:]}, {12, allRevs[4:]}, {11, allRevs[4:]}, {10, allRevs[3:]}, {9, allRevs[3:]}, {8, allRevs[2:]}, {7, allRevs[2:]}, {6, allRevs[1:]}, {5, allRevs[1:]}, {4, allRevs}, {3, allRevs}, {2, allRevs}, {1, allRevs}, {0, allRevs}}
	for i, tt := range tests {
...
func TestKeyIndexPut(t *testing.T) {
	...
	ki.put(7, 0)
	wki = &keyIndex{key: []byte("foo"), modified: revision{7, 0}, generations: []generation{{created: revision{5, 0}, ver: 2, revs: []revision{{main: 5}, {main: 7}}}}}
	if !reflect.DeepEqual(ki, wki) {
		t.Errorf("ki = %+v, want %+v", ki, wki)
	}
}
...
func TestKeyIndexTombstone(t *testing.T) {
	...
	wki = &keyIndex{key: []byte("foo"), modified: revision{15, 0}, generations: []generation{{created: revision{5, 0}, ver: 2, revs: []revision{{main: 5}, {main: 7}}}, {created: revision{8, 0}, ver: 3, revs: []revision{{main: 8}, {main: 9}, {main: 15}}}, {}}}
	if !reflect.DeepEqual(ki, wki) {
		t.Errorf("ki = %+v, want %+v", ki, wki)
	}
...
func TestKeyIndexCompactOnFurtherRev(t *testing.T) {
	...
	am := make(map[revision]struct{})
	ki.compact(3, am)
	wki := &keyIndex{key: []byte("foo"), modified: revision{2, 0}, generations: []generation{{created: revision{1, 0}, ver: 2, revs: []revision{{main: 2}}}}}
	wam := map[revision]struct{}{{main: 2}: {}}
	if !reflect.DeepEqual(ki, wki) {
		t.Errorf("ki = %+v, want %+v", ki, wki)
	}
...
func TestKeyIndexIsEmpty(t *testing.T) {
	...
	tests := []struct {
		ki	*keyIndex
		w	bool
	}{{&keyIndex{key: []byte("foo"), generations: []generation{{}}}, true}, {&keyIndex{key: []byte("foo"), modified: revision{2, 0}, generations: []generation{{created: revision{1, 0}, ver: 2, revs: []revision{{main: 2}}}}}, false}}
	for i, tt := range tests {
		g := tt.ki.isEmpty()
		if g != tt.w {
...
func TestKeyIndexFindGeneration(t *testing.T) {
	...
	tests := []struct {
		rev	int64
		wg	*generation
	}{{0, nil}, {1, nil}, {2, &ki.generations[0]}, {3, &ki.generations[0]}, {4, &ki.generations[0]}, {5, &ki.generations[0]}, {6, nil}, {7, nil}, {8, &ki.generations[1]}, {9, &ki.generations[1]}, {10, &ki.generations[1]}, {11, &ki.generations[1]}, {12, nil}, {13, nil}}
	for i, tt := range tests {
...
func TestKeyIndexLess(t *testing.T) {
	...
	tests := []struct {
		ki	*keyIndex
		w	bool
	}{{&keyIndex{key: []byte("doo")}, false}, {&keyIndex{key: []byte("foo")}, false}, {&keyIndex{key: []byte("goo")}, true}}
	for i, tt := range tests {
		g := ki.Less(tt.ki)
		if g != tt.w {
...
func TestGenerationIsEmpty(t *testing.T) {
	...
	tests := []struct {
		g	*generation
		w	bool
	}{{nil, true}, {&generation{}, true}, {&generation{revs: []revision{{main: 1}}}, false}}
	for i, tt := range tests {
		g := tt.g.isEmpty()
		if g != tt.w {
			t.Errorf("#%d: isEmpty = %v, want %v", i, g, tt.w)
...
func TestGenerationWalk(t *testing.T) {
	...
	tests := []struct {
		...
		wi	int
	}{{func(rev revision) bool {
		return rev.main >= 7
	}, 2}, {func(rev revision) bool {
		return rev.main >= 6
	}, 1}, {func(rev revision) bool {
		return rev.main >= 5
	}, 1}, {func(rev revision) bool {
		return rev.main >= 4
	}, 0}, {func(rev revision) bool {
		return rev.main >= 3
	}, 0}, {func(rev revision) bool {
		return rev.main >= 2
	}, -1}}
