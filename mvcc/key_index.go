...
func (ki *keyIndex) put(lg *zap.Logger, main int64, sub int64) {
	...
	if !rev.GreaterThan(ki.modified) {
		if lg != nil {
			lg.Panic("'put' with an unexpected smaller revision", zap.Int64("given-revision-main", rev.main), zap.Int64("given-revision-sub", rev.sub), zap.Int64("modified-revision-main", ki.modified.main), zap.Int64("modified-revision-sub", ki.modified.sub))
		} else {
...
func (ki *keyIndex) restore(lg *zap.Logger, created, modified revision, ver int64) {
	...
	if len(ki.generations) != 0 {
		if lg != nil {
			lg.Panic("'restore' got an unexpected non-empty generations", zap.Int("generations-size", len(ki.generations)))
		} else {
...
func (ki *keyIndex) tombstone(lg *zap.Logger, main int64, sub int64) error {
	...
	if ki.isEmpty() {
		if lg != nil {
			lg.Panic("'tombstone' got an unexpected empty keyIndex", zap.String("key", string(ki.key)))
		} else {
...
func (ki *keyIndex) get(lg *zap.Logger, atRev int64) (modified, created revision, ver int64, err error) {
	...
	if ki.isEmpty() {
		if lg != nil {
			lg.Panic("'get' got an unexpected empty keyIndex", zap.String("key", string(ki.key)))
		} else {
...
func (ki *keyIndex) since(lg *zap.Logger, rev int64) []revision {
	...
	if ki.isEmpty() {
		if lg != nil {
			lg.Panic("'since' got an unexpected empty keyIndex", zap.String("key", string(ki.key)))
		} else {
			plog.Panicf("store.keyindex: unexpected get on empty keyIndex %s", string(ki.key))
		}
	}
