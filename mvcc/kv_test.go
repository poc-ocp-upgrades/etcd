...
func testKVRange(t *testing.T, f rangeFunc) {
	...
	}{{[]byte("doo"), []byte("foo"), nil}, {[]byte("foo"), []byte("foo"), nil}, {[]byte("doo"), nil, nil}, {[]byte("foo"), []byte("foo3"), kvs}, {[]byte("foo"), []byte("foo1"), kvs[:1]}, {[]byte("foo"), nil, kvs[:1]}, {[]byte(""), []byte(""), kvs}}
...
func testKVRangeRev(t *testing.T, f rangeFunc) {
	...
	tests := []struct {
		...
		wrev	int64
		wkvs	[]mvccpb.KeyValue
	}{{-1, 4, kvs}, {0, 4, kvs}, {2, 4, kvs[:1]}, {3, 4, kvs[:2]}, {4, 4, kvs}}
	for i, tt := range tests {
...
func testKVRangeBadRev(t *testing.T, f rangeFunc) {
	...
	tests := []struct {
		rev	int64
		werr	error
	}{{-1, nil}, {0, nil}, {1, ErrCompacted}, {2, ErrCompacted}, {4, nil}, {5, ErrFutureRev}, {100, ErrFutureRev}}
	for i, tt := range tests {
...
func testKVRangeLimit(t *testing.T, f rangeFunc) {
	...
	tests := []struct {
		limit	int64
		wkvs	[]mvccpb.KeyValue
	}{{-1, kvs}, {0, kvs}, {1, kvs[:1]}, {2, kvs[:2]}, {3, kvs}, {100, kvs}}
	for i, tt := range tests {
...
func testKVPutMultipleTimes(t *testing.T, f putFunc) {
	...
	for i := 0; i < 10; i++ {
		...
		wkvs := []mvccpb.KeyValue{{Key: []byte("foo"), Value: []byte("bar"), CreateRevision: 2, ModRevision: base + 1, Version: base, Lease: base}}
...
func testKVDeleteRange(t *testing.T, f deleteRangeFunc) {
	...
	tests := []struct {
		...
		wrev		int64
		wN		int64
	}{{[]byte("foo"), nil, 5, 1}, {[]byte("foo"), []byte("foo1"), 5, 1}, {[]byte("foo"), []byte("foo2"), 5, 2}, {[]byte("foo"), []byte("foo3"), 5, 3}, {[]byte("foo3"), []byte("foo8"), 4, 0}, {[]byte("foo3"), nil, 4, 0}}
	for i, tt := range tests {
		b, tmpPath := backend.NewDefaultTmpBackend()
		s := NewStore(b, &lease.FakeLessor{}, nil)
...
func TestKVOperationInSequence(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		wkvs := []mvccpb.KeyValue{{Key: []byte("foo"), Value: []byte("bar"), CreateRevision: base + 1, ModRevision: base + 1, Version: 1, Lease: int64(lease.NoLease)}}
		if !reflect.DeepEqual(r.KVs, wkvs) {
			t.Errorf("#%d: kvs = %+v, want %+v", i, r.KVs, wkvs)
		}
		if r.Rev != base+1 {
...
func TestKVTxnBlockWriteOperations(t *testing.T) {
	...
	s := NewStore(b, &lease.FakeLessor{}, nil)
	tests := []func(){func() {
		s.Put([]byte("foo"), nil, lease.NoLease)
	}, func() {
...
func TestKVCompactReserveLastValue(t *testing.T) {
	...
	tests := []struct {
		rev	int64
		wkvs	[]mvccpb.KeyValue
	}{{1, []mvccpb.KeyValue{{Key: []byte("foo"), Value: []byte("bar0"), CreateRevision: 2, ModRevision: 2, Version: 1, Lease: 1}}}, {2, []mvccpb.KeyValue{{Key: []byte("foo"), Value: []byte("bar1"), CreateRevision: 2, ModRevision: 3, Version: 2, Lease: 2}}}, {3, nil}, {4, []mvccpb.KeyValue{{Key: []byte("foo"), Value: []byte("bar2"), CreateRevision: 5, ModRevision: 5, Version: 1, Lease: 3}}}}
	for i, tt := range tests {
...
func TestKVCompactBad(t *testing.T) {
	...
	tests := []struct {
		rev	int64
		werr	error
	}{{0, nil}, {1, nil}, {1, ErrCompacted}, {4, nil}, {5, ErrFutureRev}, {100, ErrFutureRev}}
	for i, tt := range tests {
		_, err := s.Compact(tt.rev)
		if err != tt.werr {
...
func TestWatchableKVWatch(t *testing.T) {
	...
	wev := []mvccpb.Event{{Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo"), Value: []byte("bar"), CreateRevision: 2, ModRevision: 2, Version: 1, Lease: 1}}, {Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo1"), Value: []byte("bar1"), CreateRevision: 3, ModRevision: 3, Version: 1, Lease: 2}}, {Type: mvccpb.PUT, Kv: &mvccpb.KeyValue{Key: []byte("foo1"), Value: []byte("bar11"), CreateRevision: 3, ModRevision: 4, Version: 2, Lease: 3}}}
...
func put3TestKVs(s KV) []mvccpb.KeyValue {
	...
	return []mvccpb.KeyValue{{Key: []byte("foo"), Value: []byte("bar"), CreateRevision: 2, ModRevision: 2, Version: 1, Lease: 1}, {Key: []byte("foo1"), Value: []byte("bar1"), CreateRevision: 3, ModRevision: 3, Version: 1, Lease: 2}, {Key: []byte("foo2"), Value: []byte("bar2"), CreateRevision: 4, ModRevision: 4, Version: 1, Lease: 3}}
}
