...
var testTLSInfo = transport.TLSInfo{KeyFile: "./fixtures/server.key.insecure", CertFile: "./fixtures/server.crt", TrustedCAFile: "./fixtures/ca.crt", ClientCertAuth: true}
...
func testServer(t *testing.T, scheme string, secure bool, delayTx bool) {
	...
	defer ln.Close()
	cfg := ServerConfig{Logger: testLogger, From: url.URL{Scheme: scheme, Host: srcAddr}, To: url.URL{Scheme: scheme, Host: dstAddr}}
	if secure {
...
func TestServer_PauseTx(t *testing.T) {
	...
	defer ln.Close()
	p := NewServer(ServerConfig{Logger: testLogger, From: url.URL{Scheme: scheme, Host: srcAddr}, To: url.URL{Scheme: scheme, Host: dstAddr}})
	<-p.Ready()
	defer p.Close()
	p.PauseTx()
	data := []byte("Hello World!")
...
func testServerHTTP(t *testing.T, secure, delayTx bool) {
	...
	srv := &http.Server{Addr: dstAddr, Handler: mux, TLSConfig: tlsConfig}
