...
func TestNewTransportTLSInfo(t *testing.T) {
	...
	defer del()
	tests := []TLSInfo{{}, {CertFile: tlsinfo.CertFile, KeyFile: tlsinfo.KeyFile}, {CertFile: tlsinfo.CertFile, KeyFile: tlsinfo.KeyFile, CAFile: tlsinfo.CAFile}, {CAFile: tlsinfo.CAFile}}
...
func TestTLSInfoEmpty(t *testing.T) {
	...
	tests := []struct {
		info	TLSInfo
		want	bool
	}{{TLSInfo{}, true}, {TLSInfo{CAFile: "baz"}, true}, {TLSInfo{CertFile: "foo"}, false}, {TLSInfo{KeyFile: "bar"}, false}, {TLSInfo{CertFile: "foo", KeyFile: "bar"}, false}, {TLSInfo{CertFile: "foo", CAFile: "baz"}, false}, {TLSInfo{KeyFile: "bar", CAFile: "baz"}, false}, {TLSInfo{CertFile: "foo", KeyFile: "bar", CAFile: "baz"}, false}}
	for i, tt := range tests {
		got := tt.info.Empty()
		if tt.want != got {
...
func TestTLSInfoMissingFields(t *testing.T) {
	...
	defer del()
	tests := []TLSInfo{{CertFile: tlsinfo.CertFile}, {KeyFile: tlsinfo.KeyFile}, {CertFile: tlsinfo.CertFile, CAFile: tlsinfo.CAFile}, {KeyFile: tlsinfo.KeyFile, CAFile: tlsinfo.CAFile}}
...
func TestTLSInfoConfigFuncs(t *testing.T) {
	...
	tests := []struct {
		...
		wantCAs		bool
	}{{info: TLSInfo{CertFile: tlsinfo.CertFile, KeyFile: tlsinfo.KeyFile}, clientAuth: tls.NoClientCert, wantCAs: false}, {info: TLSInfo{CertFile: tlsinfo.CertFile, KeyFile: tlsinfo.KeyFile, CAFile: tlsinfo.CertFile}, clientAuth: tls.RequireAndVerifyClientCert, wantCAs: true}}
	for i, tt := range tests {
